...
- For each hvac systen in the B_RMI: `for hvac in B_RMI:`  
    ...
    - for each zone in zone_list: `for zone in zone_list`  
        ...
        - Cycle though the terminal units associated with the zone to see if any have a cooling coil, set serves_cooled_zones_in_proposed to true if so: `for terminal in zone_p.terminals`  
    ...
    - Determine if exception 6 applies (Question: use minimum or maximum OA cfm?): `if ER_modeled_in_proposed == FALSE AND (Get_largest_exhaust_source_cfm (P_RMI) < 75% * hvac.fan_system.maximum_outdoor_airflow): exception_6_applies = TRUE`  
    ...
    - Case 9: Elif AHJ_RA_compare == True and OA_fraction and/or supply_cfm_b is less than 70% and/or 5,000 and energy recovery was modeled with a 50% ethalpy recovery ratio then Pass: `elif AHJ_RA_compare == True and ER_modeled == True and enthalpy_rec_ratio == 50% and (OA_fraction < 70% or supply_cfm_b < 5000): PASS`  
    - Case 10: Else, fail: `Else: outcome = "Fail"`  
