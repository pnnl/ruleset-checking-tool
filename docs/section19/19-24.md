...
- For each hvac systen in the B_RMI: `for hvac in B_RMI:`  
    ...
    - for each zone in zone_list: `for zone in zone_list`  
        ...
        - For each space in zone: `for space_p in zone_p.spaces:`
            ...
            - Check if space is of type that is likely to have systems exhausting toxic, flammable, or corrosive fumes or paint or dust, if yes then set zone is likely to have systems exhausting toxic, flammable, or corrosive fumes or paint or dust boolean variable to true based on ventilations_space_type: `if space_p.ventilations_space_type in ["MISCELLANEOUS_SPACES_MANUFACTURING_WHERE_HAZARDOUS_MATERIALS_ARE_USED_EXCLUDES_HEAVY_INDUSTRIAL_AND_CHEMICAL_PROCESSES"]:serves_zones_with_systems_likely_exhausting_toxic_etc = TRUE`          
            ...
            - Check if space is of type kitchen, if yes then set serves_kitchen_space boolean variable to true based on ventilations_space_type: `if space_p.ventilations_space_type in ["FOOD_AND_BEVERAGE_SERVICE_KITCHEN_COOKING"]:serves_kitchen_space = TRUE`                
    ...
    - Determine if exception 6 applies (Question: use minimum or maximum OA cfm?): `if ER_modeled_in_proposed == FALSE AND (list(get_hvac_sys_and_assoc_zones_largest_exhaust_source (B_RMI, hvac))[0] < 75% * hvac.fan_system.maximum_outdoor_airflow): exception_6_applies = TRUE`  
    ...
    - Case 5: Elif no energy recovery is modeled in the baseline and where the largest exhaust source is less than 75% of the design outdoor airflow adn there is only one or no exhaust fans associated with the hvac system. This exception shall only be used if exhaust air energy recovery is not used in the proposed design (exception 6): Pass   :`elif OA_fraction >= 70% and supply_cfm_b >= 5000 and ER_modeled == False and exception_6_applies == TRUE and (list(get_hvac_sys_and_assoc_zones_largest_exhaust_source (B_RMI, hvac))[1] == 0 or list(get_hvac_sys_and_assoc_zones_largest_exhaust_source (B_RMI, hvac))[1] == 1): PASS`  
    ...
    - Case 7: Elif no energy recovery is modeled in the baseline and the climate zone is 3C (exceptions 4 and 5 apply): Pass   :`elif OA_fraction >= 70% and supply_cfm_b >= 5000 and ER_modeled == False and climate_zone in ["CZ3C"]: PASS`  
    ...
    - Case 9: Elif no energy recovery is modeled in the baseline and where the largest exhaust source is less than 75% of the design outdoor airflow and there is more than one exhaust fan associated with the hvac system. This exception shall only be used if exhaust air energy recovery is not used in the proposed design (exception 6): UNDETERMINED:`elif OA_fraction >= 70% and supply_cfm_b >= 5000 and ER_modeled == False and exception_6_applies == TRUE and list(get_hvac_sys_and_assoc_zones_largest_exhaust_source (B_RMI, hvac))[1] > 1: UNDETERMINED and raise_message "Exhaust air energy recovery was not modeled in the baseline when the OA fraction is greater or equal to 70% and the supply cfm is greater or equal to 5,000 cfm. However, it appears that Section 90.1 G3.1.2.10 Exception 6 may be applicable which states, "Where the largest exhaust source is less than 75% of the design outdoor airflow. This exception shall only be used if exhaust air energy recovery is not used in the proposed design.". There are multilple exhasut fans associated with the hvac system and therefore this could not be assessed as the configuration is unknown. Conduct manual check to determine if this exception applies. If not then fail. `  
    - Case 10: Elif AHJ_RA_compare == True and OA_fraction and/or supply_cfm_b is less than 70% and/or 5,000 and energy recovery was modeled with a 50% ethalpy recovery ratio then Pass: `elif AHJ_RA_compare == True and ER_modeled == True and enthalpy_rec_ratio == 50% and (OA_fraction < 70% or supply_cfm_b < 5000): PASS`  
    - Case 11: Else, fail: `Else: outcome = "Fail"`  
