...
- For each hvac systen in the B_RMI: `for hvac in B_RMI:`  
    ...
    The next section checks if any exceptions may be applicable
    ...
    - Reset system type is heating only: `sys_type_heating_only = False`  
    - Reset serves zones that are cooled in the proposed boolean (for assessing exception 1): `serves_cooled_zones_in_proposed = FALSE`  
    ...
    - Reset energy recovery modeled in the proposed boolean variable: `ER_modeled_in_proposed = FALSE`  
    ...
    - for each zone in zone_list: `for zone in zone_list`  
        - Get the matching zone in the P_RMI (can we do this with an object instead of ID like I did here?): `zone_p = match_data_element(P_RMI,Zone,zone)`  
        ...
        - Cycle though the terminal units associated with the zone to see if it has a cooling coil, set serves_cooled_zones_in_proposed to true if so: `for terminal in zone_p.terminals`  
            - Check if there is a cooling source: `if terminal.cooling_source != Null and (terminal.cooling_source == "CHILLED_WATER" or terminal.cooling_source == "OTHER"): serves_cooled_zones_in_proposed = TRUE`    
        ...
        - Cycle through each HVAC system to see if it has a cooling coil, energy recovery, and/or heat recovery for dehumidification, if so then set appropriate boolean variable to true: `for hvac_p in hvac_sys_list:`  
            - Check if there is a cooling source (assume non_mechanical does not count as cooling): `if (hvac_p.cooling_system != Null) AND (hvac_p.cooling_system.cooling_system_type == "DIRECT_EXPANSION" or hvac_p.cooling_system.cooling_system_type == "FLUID_LOOP" or hvac_p.cooling_system.cooling_system_type == "OTHER"): serves_cooled_zones_in_proposed = TRUE`
            - Check if energy recovery is modeled in the proposed (this is for other exceptions): `hvac_p.fan_system.air_energy_recovery != Null and hvac_p.fan_system.air_energy_recovery.energy_recovery_type != "NONE": ER_modeled_in_proposed = TRUE `  
            - Check the dehumidification type if it has a cooling coil (for exception 7): `if hvac_p.cooling_system != Null AND hvac_p.cooling_system.dehumidification_type == "SERIES_HEAT_RECOVERY": serves_zones_that_have_dehumid_heat_recovery = TRUE`  
    
    - Check if system type is of a type that is heating only:  `If any(hvac.id in baseline_hvac_system_dict[sys_type] for sys_type in ["SYS-9", "SYS-9b", "SYS-10"): sys_type_heating_only = TRUE `       
    - Determine if exception 1 applies: `if serves_cooled_zones_in_proposed == FALSE AND serves_zones_heated_to_60_or_higher_in_proposed == FALSE: exception_1_applies = TRUE`  
    ...
    - Determine if exception 6 applies (use minimum or maximum OA cfm?): `if ER_modeled_in_proposed == FALSE AND (Get_largest_exhaust_source_cfm (P_RMI) < 75% * hvac.fan_system.maximum_outdoor_airflow): exception_6_applies = TRUE`  
