...
2. get_zones_and_terminal_units_served_by_hvac_sys_dict_b()   
    ...
    - For each hvac system in the B_RMI: `for hvac in B_RMI...HeatingVentilatingAirConditioningSystem:` 
        ...
        - Check if system type is of an applicable system type, if yes then carry on, if no then NOT_APPLICABLE for the hvac system:  `if any(baseline_system_type_compare(sys_type, target_sys_type, false) for target_system_type in target_system_types):`         
            ...
            - Check if the number of supply fans equals 1, if not go to rule assertion: `if get_fan_system_object_supply_return_exhaust_relief_total_power_flow(B_RMI,fan_sys_b)["supply_fans_qty"] == 1:`  
                ...
                - Get the motor efficiency from Table G3.9.1 for expected BHP (should be looked up based on the next motor size greater than the bhp (i.e., expected_BHP)): `expected_motor_efficiency = data_lookup(G3.9.1,expected_BHP)`  
                - Get the motor efficiency from Table G3.9.1 for minimum BHP (should be looked up based on the next motor size greater than the bhp (i.e., expected_BHP)) (not used): `min_motor_efficiency = data_lookup(G3.9.1,min_BHP)`  
                - Calculate the expected Wattage for expected BHP (assuming no other pressure drop adjustments apply except energy recovery and MERV filters): `expected_fan_Wattage = expected_BHP * 746 *(1/expected_motor_efficiency)`  
                - Calculate the minimim Wattage for minimim BHP (not used): `min_fan_Wattage = min_BHP * 746 *(1/min_motor_efficiency)`  


            - **Rule Assertion:** 
            - Case 1: If fan wattage expected equals total fan power: `if more_than_one_exhaust_fan_and_energy_rec_is_relevant == false and more_than_one_supply_fan == false and total_fan_power == expected_fan_Wattage: outcome = "PASS"`  
            - Case 2: Elif conservative comparison equals true and fan is less than expected outcome is Pass:`elif more_than_one_supply_fan == false and AHJ_RA_compare == True and total_fan_power < expected_fan_Wattage: PASS and raise_message "The total fan power for <insert hvac.id> is modeled as <insert total_fan_power> kW which is less than the expected including pressure drop adjustments for exhaust air energy recovery and MERV filters as applicable which was calculated as <insert min_fan_Wattage> kW. Pass because this is generally considered more conservative."`  `  
            - Case 3: Elif conservative comparison is false and fan is less than expected outcome is Fail:`elif more_than_one_supply_fan == false and AHJ_RA_compare == False and total_fan_power < expected_fan_Wattage: "FAIL" and raise_message "The total fan power for <insert hvac.id> is modeled as <insert total_fan_power> kW which is less than the expected including pressure drop adjustments for exhaust air energy recovery and MERV filters as applicable which was calculated as <insert min_fan_Wattage> kW ."`         
