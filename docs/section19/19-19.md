...
- Check if B-RMI is modeled with at least one air-side system that is of type associated with the target systems, if yes then carry on, if no then NOT_APPLICABLE for the project: `if any(baseline_system_type_compare(system_type, target_sys_type, false) for system_type in baseline_hvac_system_dict.keys() for target_system_type in target_system_types):`  
    ...
    - For each hvac system in the B_RMI: `for hvac in B_RMI...HeatingVentilatingAirConditioningSystem:` 
        ...
        - Check if system type is of an applicable system type, if yes then carry on, if no then NOT_APPLICABLE for the hvac system:  `if any(baseline_system_type_compare(sys_type, target_sys_type, false) for target_system_type in target_system_types):`         
            ...
            - Reset more_than_one_exhaust_fan_and_energy_rec_is_relevant boolean variable: `more_than_one_exhaust_fan_and_energy_rec_is_relevant = true`  
            ...
            - Check if the number of supply fans equals 1, if not go to rule assertion: `if list(get_fan_system_object_supply_return_exhaust_relief_total_kW_CFM(B_RMI,fan_sys_b)["Fan_Power"]["Fan_CFM"]["Fan_Qty"])[0] == 1:`  
                ...
                - Check if there are any energy recovery objects associated with the fan system object: `if fan_sys_b.air_energy_recovery != Null:` 
                    - Check if the number of exhaust fans equals 1, if not skip the exhaust air energy recvoery calculations: `if list(get_fan_system_object_supply_return_exhaust_relief_total_kW_CFM(B_RMI,fan_sys_b)["Fan_Power"]["Fan_CFM"]["Fan_Qty"])[2] == 1:`  
                        - Set more_than_one_exhaust_fan_and_energy_rec_is_relevant to false: `more_than_one_exhaust_fan_and_energy_rec_is_relevant = false`  
                        ...
                        - Assume that the exhaust air flow through the ERV is equal to the total exhaust cfm associated with the fan system: `ERV_Ex_air_flow = total_exhaust_cfm`  
                ...
                - Check if system type is constant volume:  `if any(baseline_system_type_compare(sys_type, target_sys_type, false) for target_system_type in CV_system_types):`     
                    ...
                    - Calculate the minimum BHP: `minimum_BHP = (0.00094 * supply_cfm_b)`   
                - Else if the system type is variable volume and not system 11: `Elif any(baseline_system_type_compare(sys_type, target_sys_type, false) for target_system_type in [HVAC_SYS.SYS_5, HVAC_SYS.SYS_6, HVAC_SYS.SYS_7, HVAC_SYS.SYS_8]): `  
                    ...
                    - Calculate the minimum BHP: `minimum_BHP = (0.0013 * supply_cfm_b)`
                - Else, the system is system 11: `Else:`  
                    ...
                    - Calculate the minimum BHP: `minimum_BHP = (0.00062 * supply_cfm_b)`

                - Get the motor efficiency from Table G3.9.1 for expected BHP (should be looked up based on the next motor size greater than the bhp (i.e., expected_BHP)): `motor_eff_ex = data_lookup(G3.9.1,expected_BHP)`  
                - Get the motor efficiency from Table G3.9.1 for minimum BHP (should be looked up based on the next motor size greater than the bhp (i.e., expected_BHP)): `motor_eff_min = data_lookup(G3.9.1,minimim_BHP)`  
                - Calculate the expected Wattage for expected BHP (assuming no other pressure drop adjustments apply except energy recovery and MERV filters): `fan_Wattage_expected = expected_BHP * 746 *(1/motor_eff_ex)`  
                - Calculate the minimim Wattage for minimim BHP: `fan_Wattage_minimim = minimim_BHP * 746 *(1/motor_eff_min)`  


            - **Rule Assertion:** 
            - Case 1: If fan wattage expected equals total fan power: `if more_than_one_exhaust_fan_and_energy_rec_is_relevant == false and more_than_one_supply_fan == false and total_fan_power == fan_Wattage_expected: outcome = "PASS"`  
            - Case 2: Elif conservative comparison equals true and fan is less than expected outcome is Pass:`elif more_than_one_supply_fan == false and AHJ_RA_compare == True and total_fan_power < fan_Wattage_expected: PASS and raise_message "The total fan power for <insert hvac.id> is modeled as <insert total_fan_power> kW which is less than the expected including pressure drop adjustments for exhaust air energy recovery and MERV filters as applicable which was calculated as <insert fan_Wattage_minimim> kW. Pass because this is generally considered more conservative."`  `  
            - Case 3: Elif conservative comparison is false and fan is less than expected outcome is Fail:`elif more_than_one_supply_fan == false and AHJ_RA_compare == False and total_fan_power < fan_Wattage_expected: "FAIL" and raise_message "The total fan power for <insert hvac.id> is modeled as <insert total_fan_power> kW which is less than the expected including pressure drop adjustments for exhaust air energy recovery and MERV filters as applicable which was calculated as <insert fan_Wattage_minimim> kW ."`         
            - Case 4: Elif conservative comparison equals true and fan wattage is less than the minimum possible then Pass: `elif more_than_one_supply_fan == false and AHJ_RA_compare == True and total_fan_power < fan_Wattage_minimum: outcome = "Pass" and raise_message "The total fan power for <insert hvac.id> is modeled as <insert total_fan_power> kW which is less than the minimum possible (i.e., less than the expected fan power assuming no pressure drop adjustments) which was calculated as <insert fan_Wattage_minimim> kW. Pass because this is generally considered more conservative."`              
            - Case 5: Elif conservative comparison equals false and fan wattage is less than the minimum possible: `elif more_than_one_supply_fan == false and AHJ_RA_compare == False and total_fan_power < fan_Wattage_minimum: outcome = "FAIL" and raise_message "The total fan power for <insert hvac.id> is modeled as <insert total_fan_power> kW which is less than the minimum possible (i.e., less than the expected fan power assuming no pressure drop adjustments) which was calculated as <insert fan_Wattage_minimim> kW."`  
            - Case 6: Elif more_than_one_supply_fan equals true: `elif more_than_one_supply_fan == true or more_than_one_exhaust_fan_and_energy_rec_is_relevant == true : outcome = "UNDETERMINED" and raise_message "<insert hvac.id> has more than one <insert "supply fan" if more_than_one_supply_fan == true> and/more than one <insert "exhaust fan" if more_than_one_exhaust_fan_and_energy_rec_is_relevant == true> associated with the HVAC system in the baseline and therefore this check could not be conducted for this HVAC sytem. Conduct manual check for compliance with G3.1.2.9."`  
            - Case 7: Else, undetermined: `Else: outcome = "UNDETERMINED" and raise_message "Fan power for <hvac.id> is greater than expected per Section and Table G3.1.2.9 assuming no pressure drop adjustments (e.g., sound attenuation, air filtration, fully ducted return when required by code, airflow control devices, carbon and other gas-phase air cleaners, coil runaround loops, evaporative humidifier/coolers in series with another cooling coil, exhaust systems serving fume hoods, and laboratory and vivarium exhaust systems in high-rise buildings) per Table 6.5.3.1-2 other than <if more_than_one_exhaust_fan_and_energy_rec_is_relevant == false then include "energy recovery" otherwise exclude energy recovery> and MERV filters defined in the RMD (if modeled). Expected Wattage = <insert fan_Wattage_expected> however not all pressure drop adjustments are able to be captured in the RMD so conduct manual check."`   
