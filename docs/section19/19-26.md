...
- For each space in the B_RMI: `for space in B_RMI...Space:`  
    ...
    - Check if each value in the design_heating_multiplier_schedule aligns with the max_infiltration_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(inf_design_heating_multiplier_sch.hourly_values):`
    ...
    - Check if each value in the design_cooling_multiplier_schedule aligns with the max_infiltration_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(inf_design_cooling_multiplier_sch.hourly_values):`
    ...
    - Check if each value in the design_heating_multiplier_schedule aligns with the occ_min_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(occ_design_heating_multiplier_sch.hourly_values):`
    ...
    - Check if each value in the design_cooling_multiplier_schedule aligns with the occ_max_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(occ_design_cooling_multiplier_sch.hourly_values):`
    ...
    - For each interior lighting object: `for int_lgt in lgting_obj_list:`  
        ...
        - Get the design_heating_multiplier_schedule: `int_lgt_design_heating_multiplier_sch = get_component_by_id(B_RMI,int_lgt.design_heating_multiplier_schedule)`  
        - Get the design_cooling_multiplier_schedule: `int_lgt_design_cooling_multiplier_sch = get_component_by_id(B_RMI,int_lgt.design_cooling_multiplier_schedule)`  
        ...
        - Check if each value in the design_heating_multiplier_schedule aligns with the int_lgt_min_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(int_lgt_design_heating_multiplier_sch.hourly_values):`
        ...
        - Check if each value in the design_cooling_multiplier_schedule aligns with the int_lgt_max_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(int_lgt_design_cooling_multiplier_sch.hourly_values):`
    ...
    - For each misc equipment object: `for misc in misc_obj_list:`  
        ...
        - Get the design_heating_multiplier_schedule: `misc_design_heating_multiplier_sch = get_component_by_id(B_RMI,misc.design_heating_multiplier_schedule)`  
        - Get the design_cooling_multiplier_schedule: `misc_design_cooling_multiplier_sch = get_component_by_id(B_RMI,misc.design_cooling_multiplier_schedule)`  
        ...
        - Check if each value in the design_heating_multiplier_schedule aligns with the misc_min_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(misc_design_heating_multiplier_sch.hourly_values):`
        ...
        - Check if each value in the design_cooling_multiplier_schedule aligns with the misc_max_value (not sure if the schedule needs to be transformed or anything, for example, what if it is an event schedule?): `for x in list(misc_design_cooling_multiplier_sch.hourly_values):`
    ...
    - **Rule Assertion:** 
    - Case 1: If the space type is not dwelling unit and all schedules pass per above then pass: `if is_dwelling_unit == false and all(inf_pass_heating, inf_pass_cooling,occ_pass_heating, occ_pass_cooling,int_lgt_pass_heating, int_lgt_pass_cooling,misc_pass_heating, misc_pass_cooling) == true: outcome = "PASS"`  
    - Case 2: Else if the space type is not dwelling unit and one or more of the schedules does not pass per above then fail: `elif is_dwelling_unit == false and all(inf_pass_heating, inf_pass_cooling,occ_pass_heating, occ_pass_cooling,int_lgt_pass_heating, int_lgt_pass_cooling,misc_pass_heating, misc_pass_cooling) == false: outcome = "FAIL" and raise_message "<Insert space.id> does not appear to have followed this rule per Section G3.1.2.2.1 for one more more of the following heating or cooling design schedules: infiltration, occupants, lighting, gas and electricity using equipment"`  
    ...
    - Case 4: Else if the space type is dwelling unit and all design heating schedules pass then the outcome is NOT_APPLICABLE (or UNDETERMINED it depends on our internal discussion with regard to assessing the exception as part of another RDS): `elif is_dwelling_unit == true and all(inf_pass_heating, occ_pass_heating, int_lgt_pass_heating, misc_pass_heating) == true: outcome = "NOT_APPLICABLE" and raise_message "<Insert space.id> appears to be a dwelling unit and cooling design schedules fall under the exception to Section G3.1.2.2.1 and were not assessed as part of this check. Note all heating design schedules passed this check for this space."`  
...
2. Jason is to add design heating and cooling multiplier schedules to the schema for occupants, infiltration, lighting, and miscellaneous. 
3. I went back and forth with how to handle dwelling units for this check. I thought it would be good to check the heating design schedules in this check since they fall under this rule. Let me know what you think. Only the cooling design schedules fall under the exception.
