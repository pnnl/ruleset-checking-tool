...
- if the calendary is_leap_year, then we need to add 24 hours to hours_this_year: `if RMD.calendar.is_leap_year: hours_this_year = hours_this_year +24`
- look at each building in the RMD: `for building in B_RMD:`
    - set a boolean is_applicable equal to false: `is_applicable = false`
    ...
    - check to see if the schedule is operating 24hrs per day, all days per year: `if sum(operation_schedule) == hours_this_year:`
      - only projects with SWH in the proposed model are interpreted as have in SWH loads.  A building must both have SWH loads and a 24hr operating schedule for this rule to be applicable.
