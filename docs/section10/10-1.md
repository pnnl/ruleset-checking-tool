...
- For each zone_p in the P_RMR: `For zone_p in P_RMR...Zone:`
    ...
    - Get the baseline system type (used to determined whether to check the preheat or heating coils in hvac_b for humidification): `hvac_sys_type_b = baseline_hvac_sys_type_ids_dict_b[list(baseline_hvac_sys_type_ids_dict_b.values()).index(hvac_b.id)]`
