...
- For each hvac system in the B_RMR: `for hvac_b in B_RMR...HeatingVentilationAirConditioningSystem:`  
    ...
    - Check if HVAC system is type 2, 3, 3a, 4, and 9 (else then the check is not applicable): `if hvac_sys_type_b in ["SYS-2", "SYS-3", "SYS-3a", "SYS-4","SYS-9"]:`  
        ...
        - Else, system type is 4:`Else:` 
            ...
            - Check if cooling capacity is <65,000 Btuh OR cooling capacity was not defined (ASSUMES that 3.4 COPnfcooling and 3.4 COPnfcooling at 47 db are equally stringent) OR the zone aggregation factor was defined and the capacity is less than 65,000 Btuh OR Zone aggregation factor was not defined: `if total_cooling_capacity_b < 65000 OR was_cool_capacity_not_defined == TRUE OR (zone_list_b[0].aggregation_factor != Null AND total_heating_capacity_b/zone_list_b[0].aggregation_factor < 65000): size_category_b = 65Btuh`  

           - Set the expected baseline efficiency for <65000 expected_baseline_eff_b = 3.4
            ...
            THE REMAIN RDS TBD DEPENDING ON ANSWER TO QUESTION REGARDING EFFICIENCY AT THE LOWEST SIZE CATEOGRY
