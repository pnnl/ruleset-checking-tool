    ...
    - Check if HVAC system is type 2, 3, 3a, 4, and 9 (else then the check is not applicable): `if hvac_sys_type_b in ["SYS-2", "SYS-3", "SYS-3a", "SYS-4","SYS-9"]:`  
        ...
        - Check if the system is system type 2: `if hvac_sys_type_b = "SYS-2":`
            ...
            - Check if efficiency was found (if modeled_efficiency_b != Null): `if modeled_efficiency_b != Null:`  
                - Check if the modeled efficiency meets the expected baseline efficiency by using the RCT compare_standard_val function, Permit_More_Stringent_Value is defined globally and is TRUE OR FALSE: `efficiency_matches_acceptable_value = compare_standard_val(Permit_More_Stringent_Value, expected_baseline_eff_b, modeled_efficiency_b, "greater than")`             
