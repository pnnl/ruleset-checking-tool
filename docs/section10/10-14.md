    ...
    - Check if HVAC system is type 2, 3, 3a, 4, and 9 (else then the check is not applicable): `if hvac_sys_type_b in ["SYS-2", "SYS-3", "SYS-3a", "SYS-4","SYS-9"]:`  
        ...
        - Check else if the system type is 3 or 3a:`elif hvac_sys_type_b = "SYS-3" or hvac_sys_type_b = "SYS-3a":`
            ...
            - Else, capacity is greater than or equal to 225,000 Btuh: `Else:`
                - Check if zone aggregation_factor does not equal Null, Else do not adjust the capacity: `if zone_list_b[0].aggregation_factor != Null:`  
                    - Define heating capacity based on the number of aggregated zones: `total_heat_capacity_b = total_heating_capacity_b/zone_list_b[0].aggregation_factor`
                - Loop through each of the efficiency_metric_types associated with the heating system until COMBUSTION_EFFICIENCY is found. Get the associated value of the efficiency from the list of efficiency_metric_values associated with the heating_system_b : `For x in range(len(heating_system_b.efficiency_metric_types)):`
        ...
        - Else, system type is 4:`Else:` 
            ...
            - Check if cooling capacity is <65,000 Btuh OR cooling capacity was not designed (THIS ASSUMES THAT THE <65000 Btuh efficiency is the most stringent since its a full load efficiency): `if total_cooling_capacity_b < 65000: size_category_b = 65Btuh`
