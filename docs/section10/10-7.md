        ...
        - Check if HVAC system is of type 1, 1b, 2, 3, 3b, 4, 5, 5b, 6, or 6b AND that is does not serve more than one floor, if meets criteria then carry on otherwise outcome is Not_Applicable: `if hvac_sys_type_b in ["SYS-1", "SYS-1b", "SYS-2", "SYS-3", "SYS-3b", "SYS-4","SYS-5", "SYS-5b", "SYS-6", "SYS-6b"] and hvac_b.id not in hvac_sys_serve_more_than_one_flr_list_b:` 
            ...
            - For each cooling_system_b in hvac_b: `for cooling_system_b in hvac_b.cooling_system:`  
                ...
                - Check else if the system type is 3, 3b and that the associated zone.aggregation_factor equal Null, this means the number of aggregated hvac zones was not defined (pass if efficiency meets a lower size category or the size category associated with the system). Assumes that the HVAC system serves one zone which is consistent with the rules of App G.:`elif (hvac_sys_type_b = "SYS-3" or hvac_sys_type_b = "SYS-3b") AND zone_list_b[0].aggregation_factor == Null:`  
                    ...
                    - Check if meets_lower_and_more_conserv_or_equiv_size_category_eff_b equals False, Else go to Rule Assertions below: `if meets_lower_and_more_conserv_or_equiv_size_category_eff_b == FALSE:`  
