...
- For each building segment in the B_RMR: `for building_segment_b in B_RMR..BuildingSegment:`
    - For each hvac system in the building segment: `for hvac_b in bulding_segment_b.heating_ventilation_air_conditioning_systems:`
        - Reset boolean variable that indicates whether the hvac system is of a type applicable to this check: `hvac_system_type_applicable = FALSE`
        - Reset boolean variable that indicates if the hvac system is type 3 or 4: `sys_type_3_or_4 = FALSE`
        - Reset boolean variable that indicates if the hvac system is type 5 or 6: `sys_type_5_or_6 = FALSE`
        ...
        - Check if boolean variable that indicates whether the hvac system is of a type applicable to this check equals true, if true proceed with check: `if hvac_system_type_applicable == TRUE:`
            - For each cooling system in the HVAC system: `for cooling_system_b in hvac_b.cooling_system:` THere should only be one cooling system - not sure if makes sense to loop through the cooling systems like this. Maybe we should check if there is more than one and if there is then the outcome should be UNDETERMINED
                ...
                - Check if system type is 5 or 6: `if hvac_sys_type_b = "SYS-5" or hvac_sys_type_b = "SYS-5b" or hvac_sys_type_b = "SYS-6" or hvac_sys_type_b = "SYS-6b": sys_type_5_or_6 = TRUE`                                        
                ...
                **Rule Assertion:**
                ...
                - Case 2: Else if, if the system type is such that multiple HVAC zones or residential spaces could be combined into a single thermal block (applies baseline HVAC System Types 3 and 4) and the baseline efficiency is required to be determined based on the  equipment capacity of the thermal block divided by the number of HVAC zones or residential spaces then outcome is UNDETERMINED: `elif sys_type_3_or_4 == TRUE and COPnf_full_load_b != full_load_efficiency_b: UNDETERMINED and raise_message "CHECK IF THE BASELINE EFFICIENCY WAS MODELED CORRECTLY BASED ON THE EQUIPMENT CAPACITY OF THE THERMAL BLOCK DIVIDED BY THE NUMBER OF HVAC ZONES OR RESIDENTIAL SPACES."`
                - Case 3: Else if, if the system is Types 5 or 6 where efficiencies shall be based on the cooling equipment capacity of a single floor: `elif sys_type_5_or_6 == TRUE and COPnf_full_load_b != full_load_efficiency_b : UNDETERMINED and raise_message "CHECK IF THE BASELINE EFFICIENCY WAS MODELED CORRECTLY BASED ON THE EQUIPMENT CAPACITY OF THE THERMAL BLOCK DIVIDED BY THE NUMBER OF FLOORS.`
