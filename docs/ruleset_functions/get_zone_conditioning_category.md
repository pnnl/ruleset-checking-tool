...
- For each zone in RMR: `for zone in RMR...zones:`
  ...
  - For each terminal serving zone: `for terminal in zone.terminals:`
    ...
    - Get HVAC system connected to terminal: `hvac_sys = terminal.served_by_heating_ventilating_air_conditioning_systems` (Note XC, will there be more than one HVAC system connecting to a terminal?)
