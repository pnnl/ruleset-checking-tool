...
- Get B-RMI system types: `baseline_hvac_system_dict = get_baseline_system_types(B-RMI)`
  ...
  - Check if B-RMI is modeled with at least one air-side system that is Type-5, 6, 7, 8, continue to rule logic: `if any(baseline_system_type_compare(system_type, target_sys_type, false) for system_type in baseline_hvac_system_dict.keys() for target_system_type in target_system_types): CHECK RULE LOGIC`
