...
- For each zone in P_RMR: `for zone_p in P_RMR...zones:`
    ...
    - For each space in zone: `for space_p in zone_p.zones:`
        - Check if space is not one of the types that has health and safety mandated minimum ventilation requirements during unoccupied hours or a computer room, if yes then set applicability flag to true: `if space_p.lighting_space_type not in [CORRIDOR_HOSPITAL,LABORATORY_EXCEPT_IN_OR_AS_A_CLASSROOM,HEALTHCARE_FACILITY_OPERATING_ROOM, COMPUTER_ROOM]:`
