...
4. get_list_of_zones_serving_specific_floor  
    ...
    - Loop through each hour of the schedules and compare across the B_RMR and P_RMR, : `for x in len(fan_schedule_b):`
        - Check if schedules matches for each hour of the year, if there is a mismatch then set the schedule_mismatch boolean variable equal to TRUE: `if fan_schedule_b.hourly_values(x) != fan_schedule_p.hourly_values(x):`  
            ...
            - Check if system type is 3 or 4: `if hvac_sys_type_b in ["SYS-3","SYS-3a","SYS-3b, SYS-4"]:`
                - Get the floor name associated with the zone: `floor_name = match_data_element(B_RMR,Zone,zone_p.id).zone_ floor_name`
                - Get list of zones associated with the floor name: `list_of_zones_serving_specific_floor = get_list_of_zones_serving_specific_floor(floor_name,B_RMR)`
