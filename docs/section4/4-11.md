...
- For each zone in the P_RMR: `for zone_p in P_RMR...Zone:`
    ...
    **Rule Assertion - IN PROGRESS UPDATE BASED ON QUESTION OUTCOME:** 
    - Case 1: For each applicable zone, if there is a fan schedule mismatch but Section G3.1.1(c) appears to apply then undetermined: `if schedule_mismatch == TRUE AND hvac_type_check == TRUE AND proposed_served_by_multizone == TRUE: UNDETERMINED and raise_message "THERE IS A FAN OPERATING SCHEDULE MISMATCH BETWEEN THE BASELINE AND PROPOSED BUT SECTION G3.1.1(c) APPEARS APPLICABLE. VERIFY THAT THE FAN OPERATING SCHEDULE IN THE BASELINE IS IN ALIGNMENT WITH THE OCCUPANCY SCHEDULES."`
    - Case 2: For each applicable zone, elif there is a fan schedule match but Section G3.1.1(c) appears to apply then fail: `Elif schedule_mismatch == FALSE AND hvac_type_check == TRUE AND proposed_served_by_multizone == TRUE: FAIL and raise_message "THE FAN OPERATING SCHEDULES MATCH BETWEEN THE BASELINE AND PROPOSED WHICH IS UNEXPECTED BECAUSE SECTION G3.1.1(c) APPEARS APPLICABLE."`
    - Case 3: For each applicable zone, elif there is no schedule mismatch is observed and baseline and proposed then pass: `if schedule_mismatch == FALSE AND proposed_served_by_multizone == FALSE And baseline_served_by_multizone == FALSE : PASS`
    - Case 4: Else, fail: `Else: FAIL and raise_message "THERE IS A FAN SCHEDULE MISMATCH BETWEEN THE BASELINE AND PROPOSED RMRs FOR THE HVAC SYSTEM(S) SERVING THIS ZONE. FAIL UNLESS TABLE G3.1 SECTION 4 BASELINE COLUMN EXCEPTIONS #1 or #2 IS APPLICABLE"`  
