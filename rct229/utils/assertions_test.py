from rct229.utils.assertions import MissingKeyException, getattr_, get_first_attr_
...
def test__get_first_attr__with_required_fields():
    assert get_first_attr_(GETFIRSTATTR_TEST_OBJ, "construction", ["u_factor", "c_factor", "f_factor"]) == 1.234

def test__get_first_attr__with_missing_fields():
    try:
        get_first_attr_(GETFIRSTATTR_TEST_OBJ, "construction", ["c_factor", "f_factor"])
    except MissingKeyException as e:
        assert str(e) == "construction:c1 is missing one of the fields in: ['c_factor', 'f_factor']"
