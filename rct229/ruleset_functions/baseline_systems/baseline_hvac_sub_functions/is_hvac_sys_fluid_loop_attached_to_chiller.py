...
from rct229.ruleset_functions.baseline_systems.baseline_system_util import (
    ...
    find_exactly_one_loop, find_exactly_one_child_loop,
)
...
def is_hvac_sys_fluid_loop_attached_to_chiller(rmi_b, hvac_b_id):
    ...
    secondary_cooling_loop_ids = [
        ...
        for secondary_loop in find_all("$.child_loops[*]", find_exactly_one_loop(rmi_b, primary_cooling_loop_id))
    ]
    ...
    if chilled_water_loop_id in primary_cooling_loop_ids:
        water_loop = find_exactly_one_loop(rmi_b, chilled_water_loop_id)
    ...
    is_hvac_sys_fluid_loop_attached_to_chiller_flag = (
        water_loop is not None
        and find_one("type", water_loop) == FLUID_LOOP.COOLING
    )
