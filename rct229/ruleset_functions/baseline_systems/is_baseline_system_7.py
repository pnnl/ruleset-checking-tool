...
def is_baseline_system_7(rmi_b, hvac_b_id, terminal_unit_id_list, zone_id_list):
    ...
    has_required_sys = (
        hvac_b.get("preheat_system") is not None and
        hvac_b.get("cooling_system") is not None and
        # heating system shall be None or heating system type shall be None (the heating_system_type =
        # HEATING_SYSTEM.NONE)
        (hvac_b.get("heating_system") is None or find_one(f'heating_system[?(@.heating_system_type={HEATING_SYSTEM.NONE})]', hvac_b) is not None)
    )

    are_sys_data_matched = (
        # sub functions handles missing required sys, and return False.
        is_hvac_sys_preheating_type_fluid_loop(rmi_b, hvac_b_id) and
        is_hvac_sys_cooling_type_fluid_loop(rmi_b, hvac_b_id) and
        is_hvac_sys_fan_sys_vsd(rmi_b, hvac_b_id) and
        does_each_zone_have_only_one_terminal(rmi_b, zone_id_list) and
        are_all_terminal_heat_sources_hot_water(rmi_b, terminal_unit_id_list) and
        are_all_terminal_cool_sources_none_or_null(rmi_b, terminal_unit_id_list) and
        are_all_terminal_fans_null(rmi_b, terminal_unit_id_list) and
        are_all_terminal_types_VAV(rmi_b, terminal_unit_id_list)
    )

    if has_required_sys and are_sys_data_matched:
        if is_hvac_sys_preheat_fluid_loop_attached_to_boiler(B_RMR,
                                                             hvac_b.id) == TRUE AND are_all_terminal_heating_loops_attached_to_boiler(B_RMR, terminal_unit_id_list) == TRUE AND is_hvac_sys_fluid_loop_attached_to_chiller(B_RMR, hvac_b.id) == TRUE: is_baseline_system_7 = "Sys-7