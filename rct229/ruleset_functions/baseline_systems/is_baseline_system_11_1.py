...
from rct229.ruleset_functions.baseline_systems.baseline_system_util import find_exact_one_hvac_system, HVAC_SYS
...
def is_baseline_system_11_1(rmi_b, hvac_b_id, terminal_unit_id_list, zone_id_list):
    ...
    has_required_sys = hvac_b.get("preheat_system") is None or find_one(f'preheat_system[?(@.heating_system_type="{HEATING_SYSTEM.NONE}")]', hvac_b,) is not None
    ...
    are_sys_data_matched = {
        ...
        and does_hvac_system_serve_single_zone(rmi_b, hvac_b_id)
        and does_each_zone_have_only_one_terminal(rmi_b, hvac_b_id)
    ...
    if are_sys_data_matched:
        ...
        elif is_hvac_sys_heating_type_fluid_loop(rmi_b, hvac_b_id) and is_hvac_sys_fluid_loop_purchased_heating(rmi_b, hvac_b_id):
            if is_hvac_sys_fluid_loop_attached_to_chiller(rmi_b, hvac_b_id):
