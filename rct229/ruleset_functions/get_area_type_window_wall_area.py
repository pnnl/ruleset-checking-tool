...
from rct229.ruleset_functions.get_opaque_surface_type import get_opaque_surface_type, ABOVE_GRADE_WALL
        ...
        for surface in find_all("zones[*].surfaces[*]", building_segment):
            if (get_opaque_surface_type(
                ...
                for subsurface in find_all("subsurfaces[*]", surface):
                    assert_("classification" in subsurface, f"subsurface:{subsurface['id']} has no classification")
                    assert_("glazed_area" in subsurface, f"subsurface:{subsurface['id']} has no glazed_area")
                    assert_("opaque_area" in subsurface, f"subsurface:{subsurface['id']} has no opaque_area")
