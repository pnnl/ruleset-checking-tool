...
from rct229.utils.assertions import getattr_, assert_, assert_required_fields
...
def normalize_space_schedules(space, zone_height, schedules):
    ...
    space_total_power_per_area = ZERO.POWER
    space_total_hourly_use_per_area_array = []

    for interior_lighting in space.get("interior_lighting", []):
        power_per_area = interior_lighting.get("power_per_area", ZERO.POWER)
        space_total_power_per_area += power_per_area

        control_credit = 0.0
        if interior_lighting["are_schedules_used_for_modeling_occupancy_control"]:
            control_credit = table_G3_7_lookup(
                ...
                occupancy_control_type=interior_lighting.get("occupancy_control_type", None),
                space_height=zone_height,
                space_area=getattr_(space, "space", "floor_area")
            )["control_credit"]

        schedule_hourly_value = getattr_(
            find_one_with_field_value("$", "id", interior_lighting["lighting_multiplier_schedule"], schedules),
            "schedule", "hourly_values")
        hourly_use_per_area_array = [hourly_value / (1.0 - control_credit) * power_per_area for hourly_value in
                                     schedule_hourly_value]

        if not space_total_hourly_use_per_area_array:
            # if the space_total_hourly_use_per_area_array is empty
            space_total_hourly_use_per_area_array = [0.0] * len(hourly_use_per_area_array)

        # make sure the hourly schedules have the same number of hours
        assert_(len(space_total_hourly_use_per_area_array) == len(hourly_use_per_area_array),
        ...
        space_total_hourly_use_per_area_array = [hourly_value_1 + hourly_value_2 for hourly_value_1, hourly_value_2 in
                                                 zip(space_total_hourly_use_per_area_array, hourly_use_per_area_array)]

    space_normalized_schedule_array = [hourly_value / space_total_power_per_area for hourly_value in space_total_hourly_use_per_area_array]
