            ...
            for row_i in range(rule_value_list.size):
                ...
                if "rmr_template" in key_list:
                    ...
                    if "json_template" in key_list:
                        ...
                        for rmr_triplet in triplet_strs:
                            ...
                            if (
                                ...
                                if rmr_template_dict[test_id]["rmr_template"][rmr_triplet]!= True:
                                    continue
                        ...
                        key_list = [item for item in key_list if item not in keys_to_ignore]
                    ...
                    if not zones_have_been_mapped and system_to_zone_dict:
                        zones_have_been_mapped = True # Set to true to avoid setting them again
...
def set_systems_to_zones(json_dict, system_to_zone_dict, rule_set):
    ...
    for system in system_to_zone_dict["system_zone_assignment"]["systems"]:
        ...
        system_type_path = os.path.join(file_dir, "..", rule_set, "system_types", f"{system_name}.json")
        ...
        add_baseline_terminals(json_dict, system_rmd, system_classification, system_name, zone_list)

        # Adjust building segment HVACs
        add_hvac_systems(json_dict, system_rmd, system_classification, system_name, zone_list)
...
def get_rmr_triplet_dicts_from_ruletest_json_dict(ruletest_json_dict):
    ...
    for triplet in triplet_strs:
        if triplet in ruletest_json_dict[test_id]["rmr_transformations"]:
            rmr_triplet_dict_list.append(ruletest_json_dict[test_id]["rmr_transformations"][triplet])

    return rmr_triplet_dict_list


def add_baseline_terminals(template_rmd, system_rmd, system_classification, system_name, zone_list):
    ...
    terminal_keys = get_json_path_key_list_from_enumeration(json_path_enumeration="terminals")
    terminal_keys[-1] = terminal_keys[-1] + "[0]" # append a [0] to final terminals key to reference first terminal
    ...
    for rmr_instance_dict in rmr_triplet_dict_list:
        ...
        zone_keys = get_json_path_key_list_from_enumeration(json_path_enumeration="zones")
        ...
        for zone in zones:
            ...
            if zone["id"] in zone_list:
                ...
                if system_classification != "MultiZoneAirLoop":
                    ...
                    system_name = clean_system_name_for_multizone_airloop(rmr_instance_dict, system_name)
    ...
    # If not a list, it should be a dict, but check. Check dictionary for IDs and iterate their value by 1
        ...
        if "id" in iterable_object:
            ...
            if iterable_object["id"][-1].isdigit():
                ...
                iterable_object["id"] = iterable_object["id"][:-1] + str(final_index + 1)
...
def add_hvac_systems(template_rmd, system_rmd, system_classification, system_name, zone_list):
    ...
    hvac_keys = get_json_path_key_list_from_enumeration(json_path_enumeration="heating_ventilating_air_conditioning_systems")
    hvac_keys[-1] = hvac_keys[-1] + "[0]"  # append a [0] to final terminals key to reference first terminal
    ...
    for rmr_instance_dict in rmr_triplet_dict_list:
        ...
        building_segment_keys = get_json_path_key_list_from_enumeration(json_path_enumeration="building_segments")
        building_segments = get_nested_dic_from_key_list(rmr_instance_dict, building_segment_keys)
        ...
        for building_segment in building_segments:
            ...
            if segment_contains_relevant_zone:
                ...
                if "heating_ventilating_air_conditioning_systems" not in building_segment:
                ...
                if system_classification == "MultiZoneAirLoop":
                    ...
                    for hvac_system in building_segment["heating_ventilating_air_conditioning_systems"]:
                        if system_name == hvac_system["id"]:
                    ...
                    if not hvac_system_id_set:
                        # Adjust system name
                        hvac_system = building_segment["heating_ventilating_air_conditioning_systems"][-1]
...
def add_plant_loop_equipment(template_rmd, system_rmd):
    ...
    for rmr_instance_dict in rmr_triplet_dict_list:
        ...
        template_ruleset_model_instance = rmr_instance_dict["ruleset_model_instances"][0]
        ...
        for equipment in equipment_str_list:
            ...
            if equipment in sys_ruleset_model_instance:
                ...
                if equipment in template_ruleset_model_instance:
                    ...
                    for template_equipment_instance in template_ruleset_model_instance[equipment]:
                        template_equipment_id_list.append(template_equipment_instance["id"])
                    ...
                    for system_equipment_instance in sys_ruleset_model_instance[equipment]:

                        if system_equipment_instance["id"] not in template_equipment_id_list:
                            template_ruleset_model_instance[equipment].append(system_equipment_instance)

                # If this equipment type didn't exist in the template, simply copy from system RMD
                else:
                    template_ruleset_model_instance[equipment] = sys_ruleset_model_instance[equipment]
