...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_swh_uses_associated_with_each_building_segment import get_swh_uses_associated_with_each_building_segment
...
class PRM9012019Rule06k20(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule06k20, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=True), each_rule=PRM9012019Rule06k20.RMDRule(),
            index_rmd=BASELINE_0, id='section11rule15', description=
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019Rule06k20.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=True), each_rule=PRM9012019Rule06k20.RMDRule
                .SWHUseRule(), index_rmd=BASELINE_0, list_path=
        ...
        def is_applicable(self, context, data=None):
            ...
            for building_segment_b in find_all(
                ...
                swh_use_ids += [swh_use_b['id'] for swh_use_b in
                    swh_use_list_b if swh_use_b['id'] not in swh_use_ids]
            for building_segment_p in find_all(
                '$.buildings[*].building_segments[*]', rmd_p):
                swh_use_list_p = (
                    get_swh_uses_associated_with_each_building_segment(
                    rmd_p)[building_segment_p['id']])
                swh_use_ids += [swh_use_p['id'] for swh_use_p in
                    swh_use_list_p if swh_use_p['id'] not in swh_use_ids]
        ...
        def create_data(self, context, data):
            ...
            return {'swh_dist_sys_dict_b': swh_dist_sys_dict_b,
        ...
        class SWHUseRule(RuleDefinitionBase):
            ...
            def get_calc_vals(self, context, data=None):
                ...
                swh_use_served_by_distribution_system_b = swh_dist_sys_dict_b[
                    ...
                    ] if swh_use_served_by_distribution_system_id_b else None
                swh_use_served_by_distribution_system_p = swh_dist_sys_dict_p[
                    swh_use_served_by_distribution_system_id_p
                    ] if swh_use_served_by_distribution_system_id_p else None
                get_swh_dist_sys_b = getEither(
                    swh_use_served_by_distribution_system_b)
                get_swh_dist_sys_p = getEither(
                ...
                swh_use_design_supply_water_temperature_p = get_swh_dist_sys_p(
                    'design_supply_water_temperature')
                return {'is_swh_use_none_b': swh_use_b is None,
                    'is_swh_use_none_p': swh_use_p is None, 'swh_use_b':
                    get_swh_use_b('use'), 'swh_use_p': get_swh_use_p('use'),
                    'swh_use_id_b': swh_use_id_b, 'swh_use_id_p':
                    swh_use_id_p, 'swh_use_use_units_b': get_swh_use_b(
                    'use_units'), 'swh_use_use_units_p': swh_use_p.get(
                    'use_units'), 'swh_use_multiplier_schedule_b':
                    get_swh_use_b('use_multiplier_schedule'),
                    ...
                    get_swh_use_p('is_recovered_heat_used_by_cold_side_feed')}

            def manual_check_required(self, context, calc_vals=None, data=None
                ):
                swh_use_b = calc_vals['swh_use_b'] if calc_vals['swh_use_b'
                    ] else 0.0
                swh_use_p = calc_vals['swh_use_p'] if calc_vals['swh_use_p'
                    ] else 0.0
                ...
                swh_use_design_supply_water_temperature_p = calc_vals[
                    'swh_use_design_supply_water_temperature_p']
                is_heat_recovered_by_drain_b = calc_vals[
                    'is_heat_recovered_by_drain_b']
                is_heat_recovered_by_drain_p = calc_vals[
                    'is_heat_recovered_by_drain_p']
                is_recovered_heat_used_by_cold_side_feed_b = calc_vals[
                    'is_recovered_heat_used_by_cold_side_feed_b']
                is_recovered_heat_used_by_cold_side_feed_p = calc_vals[
                    'is_recovered_heat_used_by_cold_side_feed_p']
                return (is_swh_use_none_b == is_swh_use_none_p and 
                    swh_use_use_units_b == swh_use_use_units_p and 
                    swh_use_multiplier_schedule_b ==
                    swh_use_multiplier_schedule_p and 
                    swh_use_temperature_at_fixture_b ==
                    swh_use_temperature_at_fixture_p and 
                    swh_use_water_mains_temperature_schedule_b ==
                    swh_use_water_mains_temperature_schedule_p and 
                    swh_use_served_by_distribution_system_b ==
                    swh_use_served_by_distribution_system_p and 
                    swh_use_design_supply_water_temperature_b ==
                    swh_use_design_supply_water_temperature_p and 
                    is_heat_recovered_by_drain_b ==
                    is_heat_recovered_by_drain_p and 
                    is_recovered_heat_used_by_cold_side_feed_b ==
                    is_recovered_heat_used_by_cold_side_feed_p and 
                    swh_use_b > swh_use_p)
            ...
            def rule_check(self, context, calc_vals=None, data=None):
                ...
                if (not is_swh_use_none_b and is_swh_use_none_p or 
                    ...
                    if swh_use_use_units_b != swh_use_use_units_p:
                        rule_result = False
                    if (swh_use_multiplier_schedule_b !=
                        swh_use_multiplier_schedule_p):
                        rule_result = False
                    if (swh_use_temperature_at_fixture_b !=
                        swh_use_temperature_at_fixture_p):
                        rule_result = False
                    if (swh_use_water_mains_temperature_schedule_b !=
                        swh_use_water_mains_temperature_schedule_p):
                        rule_result = False
                    if (is_heat_recovered_by_drain_b !=
                        is_heat_recovered_by_drain_p):
                        rule_result = False
                    if (is_recovered_heat_used_by_cold_side_feed_b !=
                        is_recovered_heat_used_by_cold_side_feed_p):
                        rule_result = False
                    if (swh_use_served_by_distribution_system_b !=
                        swh_use_served_by_distribution_system_p):
                        rule_result = False
                    if (swh_use_design_supply_water_temperature_b !=
                        swh_use_design_supply_water_temperature_p):
                        rule_result = False
            ...
            def get_fail_msg(self, context, calc_vals=None, data=None):
                ...
                if not is_swh_use_none_b and is_swh_use_none_p:
                    rule_note = (
                        f'{swh_use_id_p} exists in the proposed model, but not in the baseline.'
                        )
                elif is_swh_use_none_b and not is_swh_use_none_p:
                    rule_note = (
                        f'{rule_note} {swh_use_id_b} exists in the baseline, but not in the proposed model.'
                        )
                else:
                    if swh_use_use_units_b != swh_use_use_units_p:
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Service water heating use units are inconsistent between proposed and baseline models.'
                            )
                    if (swh_use_multiplier_schedule_b !=
                        swh_use_multiplier_schedule_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Service Water Heating Use schedules do not match.'
                            )
                    if (swh_use_temperature_at_fixture_b !=
                        swh_use_temperature_at_fixture_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} the temperature at fixture is not the same between Proposed and Baseline.'
                            )
                    if (swh_use_water_mains_temperature_schedule_b !=
                        swh_use_water_mains_temperature_schedule_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Service Water Heating Distribution System entering main water temperature schedules do not match.'
                            )
                    if (is_heat_recovered_by_drain_b !=
                        is_heat_recovered_by_drain_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Heat recovered by drain do not match.'
                            )
                    if (is_recovered_heat_used_by_cold_side_feed_b !=
                        is_recovered_heat_used_by_cold_side_feed_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Recovered heat used by cold side feed do not match.'
                            )
                    if (swh_use_served_by_distribution_system_b !=
                        swh_use_served_by_distribution_system_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Service water heating distribution system that serves this water heating use units are inconsistent between proposed and baseline models.'
                            )
                    if (swh_use_design_supply_water_temperature_b !=
                        swh_use_design_supply_water_temperature_p):
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Service Water Heating Distribution System design water supply temperatures do not match.'
                            )
                    if swh_use_b < swh_use_p:
                        rule_note = (
                            f'{rule_note} {swh_use_id_b} Proposed Service Water Heating Use is greater than the baseline.'
                            )
                return rule_note
