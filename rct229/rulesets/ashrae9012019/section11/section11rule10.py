...
INSTANTANEOUS_TYPES = [SWHTankOptions.CONSUMER_INSTANTANEOUS,
    SWHTankOptions.COMMERCIAL_INSTANTANEOUS, SWHTankOptions.
    RESIDENTIAL_DUTY_COMMERCIAL_INSTANTANEOUS]
STORAGE_TYPES = [SWHTankOptions.CONSUMER_STORAGE, SWHTankOptions.
    COMMERCIAL_STORAGE]
draw_pattern_enum_to_lookup_str_map = {DrawPatternOptions.VERY_SMALL:
    'Very small', DrawPatternOptions.LOW: 'Low', DrawPatternOptions.MEDIUM:
    'Medium', DrawPatternOptions.HIGH: 'High'}
...
class PRM9012019Rule76q85(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule76q85, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule76q85.RMDRule(),
            index_rmd=BASELINE_0, id='section11rule10', description=
            'The service water heating system type in the baseline building design shall match the minimum efficiency requirements in Section 7.4.2.'
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019Rule76q85.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), each_rule=PRM9012019Rule76q85.
                RMDRule.SWHEquipRule(), index_rmd=BASELINE_0, list_path=
        ...
        class SWHEquipRule(RuleDefinitionBase):
            ...
            def __init__(self):
                super(PRM9012019Rule76q85.RMDRule.SWHEquipRule, self).__init__(
                    ...
                    ['tank', 'input_power', 'efficiency_metric_types',
                    'efficiency_metric_values'], 'tank': ['type',
                    'storage_capacity']}, precision={'swh_efficiency_b': {
                    'precision': 0.01, 'unit': ''}, 'swh_efficiency_uef_b':
                    {'precision': 0.0001, 'unit': ''},
                    'swh_standby_loss_fraction_b': {'precision': 0.01,
                    'unit': ''}, 'swh_standby_loss_energy_b': {'precision':
                    1, 'unit': 'Btu/h'}})
            ...
            def get_calc_vals(self, context, data=None):
                ...
                swh_efficiency_b = dict(zip(swh_equip_b[
                    'efficiency_metric_types'], swh_equip_b[
                    'efficiency_metric_values']))
                swh_input_power_per_volume_b = (swh_input_power_b /
                ...
                draw_pattern_b = '' if swh_input_power_b > 105000 * ureg(
                    'Btu/h') else draw_pattern_b
                ...
                if (swh_fuel_type_b == EnergySourceOptions.ELECTRICITY and 
                    ...
                    if swh_input_power_b <= 75000 * ureg('Btu/h'):
                        ...
                        efficiency_data = table_7_8_lookup(
                            'Gas storage water heater', swh_input_power_b,
                            draw_pattern_b)
                if efficiency_data:
                    to_remove = None
                    for efficiency in efficiency_data:
                        if 'STANDBY' in efficiency['metric']:
                            standby_loss_target_b = eval(efficiency[
                                'equation'], {'__builtins__': None}, {'v':
                                swh_tank_storage_volume_b.to('gallon').
                                magnitude, 'q': swh_input_power_b.to(
                                'Btu/h').magnitude})
                    ...
                    if len(efficiency_data) == 1:
                        expected_efficiency_metric_b = efficiency_data[0][
                            'metric']
                        expected_efficiency_b = eval(efficiency_data[0][
                            'equation'], {'__builtins__': None}, {'v':
                            swh_tank_storage_volume_b.to('gallon').
                            magnitude, 'q': swh_input_power_b.to('Btu/h').
                            magnitude})
                        modeled_efficiency_b = swh_efficiency_b.get(
                            expected_efficiency_metric_b)
                return {'swh_fuel_type_b': swh_fuel_type_b,
                    'swh_tank_type_b': swh_tank_type_b, 'swh_input_power_b':
                    swh_input_power_b, 'swh_tank_storage_volume_b': CalcQ(
                    ...
                    'expected_efficiency_metric_b':
                    expected_efficiency_metric_b, 'standby_loss_target_b':
                    standby_loss_target_b, 'standby_loss_target_metric_b':
                    standby_loss_target_metric_b}
            ...
            def manual_check_required(self, context, calc_vals=None, data=None
                ...
                swh_input_power_per_volume_b = calc_vals[
                    'swh_input_power_per_volume_b']
                swh_tank_storage_volume_b = calc_vals[
                    'swh_tank_storage_volume_b']
                modeled_efficiency_b = calc_vals['modeled_efficiency_b']
                modeled_standby_loss_b = calc_vals['modeled_standby_loss_b']
                expected_efficiency_metric_b = calc_vals[
                    'expected_efficiency_metric_b']
                standby_loss_target_metric_b = calc_vals[
                    'standby_loss_target_metric_b']
                return (swh_tank_type_b in INSTANTANEOUS_TYPES or 
                    swh_input_power_per_volume_b >
                    CAPACITY_PER_VOLUME_LIMIT or swh_fuel_type_b ==
                    EnergySourceOptions.ELECTRICITY and swh_tank_type_b in
                    STORAGE_TYPES and 55 * ureg('gallon') <
                    swh_tank_storage_volume_b <= 100 * ureg('gallon') or 
                    expected_efficiency_metric_b is None and 
                    standby_loss_target_metric_b is None or 
                    modeled_efficiency_b is None and modeled_standby_loss_b is
                    None or expected_efficiency_metric_b and
                    standby_loss_target_metric_b and (modeled_efficiency_b is
                    None or modeled_standby_loss_b is None))
            ...
            def get_manual_check_required_msg(self, context, calc_vals=None,
                ...
                if (swh_fuel_type_b == EnergySourceOptions.ELECTRICITY and 
                    swh_tank_type_b in STORAGE_TYPES and 55 * ureg('gallon'
                ...
                elif swh_fuel_type_b == EnergySourceOptions.ELECTRICITY and swh_tank_type_b in STORAGE_TYPES and swh_input_power_b <= 12 * ureg(
                    'kW') and (swh_tank_storage_volume_b > 100 * ureg(
                    'gallon') or swh_tank_storage_volume_b < 20 * ureg(
                    'gallon')):
                    manual_check_msg.append(
                    ...
                    EnergySourceOptions.PROPANE
                    ] and swh_tank_type_b in STORAGE_TYPES and swh_input_power_b <= 75000 * ureg(
                    'Btu/h') and (swh_tank_storage_volume_b > 100 * ureg(
                    'gallon') or swh_tank_storage_volume_b < 20 * ureg(
                    'gallon')):
                    manual_check_msg.append(
                        'The storage tank volume falls outside the supported range based on the size categories in ASHRAE 90.1 Table 7.8 and in 10 CFR 430. Consequently, this rule was not assessed for this service water heater. Note that the specific requirements of 10 CFR 430 can be found in ASHRAE 90.1 Appendix F Table F-2.'
                ...
                if (expected_efficiency_metric_b is not None and 
                ...
                if (standby_loss_target_metric_b is not None and 
                    modeled_standby_loss_b is None):
                    manual_check_msg.append(
            ...
            def rule_check(self, context, calc_vals=None, data=None):
                ...
                standby_loss_target_metric_b = calc_vals[
                    'standby_loss_target_metric_b']
                precision_entry = ('swh_efficiency_b' if 
                    expected_efficiency_metric_b ==
                    SWHEfficiencyMetricOptions.THERMAL_EFFICIENCY else
                    'swh_efficiency_uef_b')
                standby_loss_complies = standby_loss_target_b is None or (
                    standby_loss_target_metric_b ==
                    SWHEfficiencyMetricOptions.STANDBY_LOSS_FRACTION and
                    self.precision_comparison['swh_standby_loss_fraction_b'
                    ](modeled_standby_loss_b, standby_loss_target_b) or 
                    standby_loss_target_b is not None and 
                    modeled_standby_loss_b < standby_loss_target_b) or (
                    standby_loss_target_metric_b ==
                    SWHEfficiencyMetricOptions.STANDBY_LOSS_ENERGY and self
                    .precision_comparison['swh_standby_loss_energy_b'](
                    modeled_standby_loss_b * ureg('W'), 
                    standby_loss_target_b * ureg('Btu/h')) or 
                    standby_loss_target_b is not None and 
                    modeled_standby_loss_b * ureg('W') < 
                    standby_loss_target_b * ureg('Btu/h'))
                return swh_tank_type_b in STORAGE_TYPES and (
                    expected_efficiency_b is None or self.
                    precision_comparison[precision_entry](
            ...
            def get_fail_msg(self, context, calc_vals=None, data=None):
                swh_fuel_type_b = calc_vals['swh_fuel_type_b']
                swh_tank_type_b = calc_vals['swh_tank_type_b']
                if (swh_tank_type_b not in STORAGE_TYPES +
                    INSTANTANEOUS_TYPES or swh_fuel_type_b not in [
                    EnergySourceOptions.ELECTRICITY, EnergySourceOptions.
                    NATURAL_GAS, EnergySourceOptions.PROPANE]):
                    return (
                        'The water heater type was not recognized, and does not match any of the expected baseline water heater types.'
                        )
                else:
                    return (
                        'The modeled efficiency or standby loss for the water heater does not match the expected values.'
                        )
            ...
            def is_tolerance_fail(self, context, calc_vals=None, data=None):
                ...
                standby_loss_target_b = calc_vals['standby_loss_target_b']
                standby_loss_complies = (standby_loss_target_b is None or
                    std_equal(modeled_standby_loss_b, standby_loss_target_b
                    ) or standby_loss_target_b is not None and std_le(val=
                    modeled_standby_loss_b, std_val=standby_loss_target_b))
                return swh_tank_type_b in STORAGE_TYPES and std_equal(
                    modeled_efficiency_b, expected_efficiency_b
                    ) and standby_loss_complies
