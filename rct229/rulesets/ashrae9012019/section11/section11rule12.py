...
class PRM9012019rule52y79(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019rule52y79, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=True), each_rule=PRM9012019rule52y79.RMDRule(),
            index_rmd=BASELINE_0, id='section11rule12', description=
            'For large, 24-hour-per-day facilities that meet the prescriptive criteria for use of condenser heat recovery systems described in Section 6.5.6.2, a system meeting the requirements of that section shall be included in the baseline building design regardless of the exceptions to Section 6.5.6.2.'
            , ruleset_section_title='Service Water Heating',
            standard_section=
            'Table G3.1 #11, baseline column, d + exception',
            is_primary_rule=False, list_path=
            'ruleset_model_descriptions[0]', data_items={'is_leap_year_b':
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019rule52y79.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=True), each_rule=PRM9012019rule52y79.RMDRule
                .BuildingRule(), index_rmd=BASELINE_0, list_path=
                '$.buildings[*]')
        ...
        def create_data(self, context, data):
            ...
            return {'is_leap_year_b': data['is_leap_year_b'], 'schedules_b':
                find_all('$.schedules[*]', rmd_b), 'schedules_p': find_all(
                '$.schedules[*]', rmd_p),
                'swh_uses_associated_with_each_building_segment_p':
                swh_uses_associated_with_each_building_segment_p}
        ...
        class BuildingRule(PartialRuleDefinition):
            ...
            def is_applicable(self, context, data=None):
                ...
                bldg_open_sch_id_b = getattr_(building_b, 'buildings',
                    'building_open_schedule')
                bldg_open_sch_b = next((schedule_b['hourly_values'] for
                    schedule_b in schedules_b if schedule_b['id'] ==
                    bldg_open_sch_id_b), [])
                hours_this_year = (LeapYear.LEAP_YEAR_HOURS if
                    is_leap_year_b else LeapYear.REGULAR_YEAR_HOURS)
                return (service_water_heating_uses_p and 
                    building_open_schedule_id_p is not None and sum(
                    bldg_open_sch_b) == hours_this_year)

            def get_calc_vals(self, context, data=None):
                swh_uses_associated_with_each_building_segment_p = data[
                    'swh_uses_associated_with_each_building_segment_p']
                uses_associated_with_each_building_segment_p = {bldg_seg_id:
                    sum(swh_uses.get('use', 0.0) for swh_uses in
                    swh_uses_associated_with_each_building_segment_p[
                    bldg_seg_id]) for bldg_seg_id in
                    swh_uses_associated_with_each_building_segment_p}
                return {'uses_associated_with_each_building_segment_p':
            ...
            def applicability_check(self, context, calc_vals, data):
                uses_associated_with_each_building_segment_p = calc_vals[
                    'uses_associated_with_each_building_segment_p']
                return sum(list(
                    uses_associated_with_each_building_segment_p.values())
                    ) > 0.0
