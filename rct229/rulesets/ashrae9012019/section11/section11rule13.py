...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_energy_required_to_heat_swh_use import get_energy_required_to_heat_swh_use
...
APPLICABILITY_MSG = (
    'This building has service water heating loads.  Confirm that service water heating energy consumption is calculated explicitly based upon the volume of service water heating required and the entering makeup water and leaving service water heating temperatures.  Entering water temperatures shall be estimated based upon the location. Leaving temperatures shall be based upon the end-use requirements.'
    )
...
class PRM9012019Rule51s51(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule51s51, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule51s51.RMDRule(),
            index_rmd=BASELINE_0, id='section11rule13', description=
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019Rule51s51.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), index_rmd=BASELINE_0, each_rule=
                PRM9012019Rule51s51.RMDRule.BuildingRule(), list_path=
                '$.buildings[*]')
            ...
            for building_segment in find_all(
                ...
                for swh_use in service_water_heating_use_list:
                    ...
                    energy_required_to_heat_swh_use = (
                        get_energy_required_to_heat_swh_use(swh_use['id'],
                        rmd_b, building_segment['id'], is_leap_year_b))
                    if swh_use['id'
                        ] not in energy_required_to_heat_swh_use_dict:
                        energy_required_to_heat_swh_use_dict[swh_use['id']] = {
                            }
                    energy_required_to_heat_swh_use_dict[swh_use['id']][
                        building_segment['id']
                        ] = energy_required_to_heat_swh_use
            return {'energy_required_to_heat_swh_use_dict':
                energy_required_to_heat_swh_use_dict,
        ...
        class BuildingRule(PartialRuleDefinition):
            ...
            def get_calc_vals(self, context, data=None):
                ...
                service_water_heating_use_dict = data[
                    'service_water_heating_use_ids_dict']
                service_water_heating_info = {'btu_per_year': ZERO.ENERGY,
                    ...
                    for swh_use in service_water_heating_use_dict[
                        ...
                        if swh_use.get('use', 0.0) > 0:
                            ...
                            energy_required_by_space = (
                                energy_required_to_heat_swh_use_dict[
                                swh_use['id']][building_segment['id']])
                            if None in energy_required_by_space.values():
                                service_water_heating_info['btu_per_year'
                                    ] = 'UNDETERMINED'
                                service_water_heating_info[
                                    'btu_per_sf_per_year'] = 'UNDETERMINED'
                            else:
                                service_water_heating_info['btu_per_year'
                                    ] += sum(energy_required_by_space.values())
                if service_water_heating_info['btu_per_year'
                    ] != 'UNDETERMINED' and service_water_heating_info[
                    'btu_per_year'] != ZERO.ENERGY:
                    floor_area = sum(find_all(
                        '$.building_segments[*].zones[*].spaces[*].floor_area',
                        building_b), ZERO.AREA)
                    assert_(floor_area > ZERO.AREA,
                        f"Floor area for building: {building_b['id']} is 0, check inputs."
                        )
                    service_water_heating_info['btu_per_sf_per_year'
                        ] = service_water_heating_info['btu_per_year'
                        ] / floor_area
                return {'is_applicable': is_applicable,
                    'service_water_heating_btu_per_sf_per_year':
                    service_water_heating_info['btu_per_sf_per_year']}
