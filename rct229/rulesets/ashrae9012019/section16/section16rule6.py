...
class PRM9012019Rule34h06(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule34h06, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=True), each_rule=PRM9012019Rule34h06.ElevatorRule(),
            index_rmd=BASELINE_0, id='section16rule6', description=
            'When included in the proposed design, the baseline elevator cab ventilation fan power shall be 0.33 W/cfm.'
            , ruleset_section_title='Elevators', standard_section=
            'Table G3.1-16 Baseline Building Performance', is_primary_rule=
            True, list_path=
            'ruleset_model_descriptions[0].buildings[*].elevators[*]')
    ...
    class ElevatorRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule34h06.ElevatorRule, self).__init__(rmds_used
        ...
        def get_calc_vals(self, context, data=None):
            ...
            assert_(elevator_cab_ventilation_fan_power_b > ZERO.POWER,
                'Elevator cab ventilation fan power should be greater than 0 W.'
                )
            elevator_cab_ventilation_fan_flow_b = elevator_b[
                'cab_ventilation_fan_flow']
            assert_(elevator_cab_ventilation_fan_flow_b > ZERO.FLOW,
                'Elevator cab ventilation fan flow rate should be greater than 0 cfm.'
                )
            return {'elevator_cab_ventilation_fan_power_b': CalcQ(
        ...
        def rule_check(self, context, calc_vals=None, data=None):
            ...
            return std_equal(elevator_cab_ventilation_fan_power_b /
                elevator_cab_ventilation_fan_flow_b,
