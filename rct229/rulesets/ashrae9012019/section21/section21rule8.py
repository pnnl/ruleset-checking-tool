...
class PRM9012019Rule58s22(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule58s22, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule58s22.
            HeatingFluidLoopRule(), index_rmd=BASELINE_0, id=
            'section21rule8', description=
            'When the baseline building requires boilers, (for baseline system type = 1,5,7,11 and 12), HWST for the baseline building shall be reset using an outdoor air dry-bulb reset schedule. 180F at 20F OAT, 150Fat 50F OAT, ramped linearly between 150F and 180F.'
            , ruleset_section_title='HVAC - Water Side', standard_section=
            'Section G3.1.3.3 Building System-Specific Modeling Requirements for the Baseline model'
    ...
    def is_applicable(self, context, data=None):
        ...
        boiler_fluid_loops_set_b = set(boiler_loop_id for boiler_loop_id in
            boiler_loop_ids_b if boiler_loop_id in
            hw_loop_zone_list_w_area_dict_b)
    ...
    class HeatingFluidLoopRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule58s22.HeatingFluidLoopRule, self).__init__(
                ...
                'heating_design_and_control'], 'heating_design_and_control':
                ...
                'loop_supply_temperature_at_outdoor_low']}, precision={
                ...
                'precision': 0.1, 'unit': 'K'},
                'design_supply_temperature_at_outdoor_high_b': {'precision':
                0.1, 'unit': 'K'},
                'design_supply_temperature_at_outdoor_low_b': {'precision':
                0.1, 'unit': 'K'}})

        def get_calc_vals(self, context, data=None):
            fluid_loop_b = context.BASELINE_0
            temperature_reset_type_b = fluid_loop_b[
                'heating_design_and_control']['temperature_reset_type']
            design_outdoor_high_for_loop_supply_reset_temperature_b = (
                fluid_loop_b['heating_design_and_control'][
                'outdoor_high_for_loop_supply_reset_temperature'])
            design_outdoor_low_for_loop_supply_reset_temperature_b = (
                fluid_loop_b['heating_design_and_control'][
                'outdoor_low_for_loop_supply_reset_temperature'])
            ...
            return {'temperature_reset_type_b': temperature_reset_type_b,
                'design_outdoor_high_for_loop_supply_reset_temperature_b':
                CalcQ('temperature',
                design_outdoor_high_for_loop_supply_reset_temperature_b),
                'design_outdoor_low_for_loop_supply_reset_temperature_b':
                ...
                CalcQ('temperature', DESIGN_OUTDOOR_TEMP_FOR_RESET_HIGH),
                'required_outdoor_low_for_loop_supply_reset_temperature_b':
                CalcQ('temperature', DESIGN_OUTDOOR_TEMP_FOR_RESET_LOW),
        ...
        def rule_check(self, context, calc_vals=None, data=None):
            ...
            return (temperature_reset_type_b == DESIGN_TEMP_RESET_TYPE.
                OUTSIDE_AIR_RESET and self.precision_comparison[
                'design_outdoor_high_for_loop_supply_reset_temperature_b'](
                design_outdoor_high_for_loop_supply_reset_temperature_b.to(
                ureg.kelvin),
                required_outdoor_high_for_loop_supply_reset_temperature_b.
                to(ureg.kelvin)) and self.precision_comparison[
                'design_outdoor_low_for_loop_supply_reset_temperature_b'](
                design_outdoor_low_for_loop_supply_reset_temperature_b.to(
                ureg.kelvin),
                required_outdoor_low_for_loop_supply_reset_temperature_b.to
                (ureg.kelvin)) and self.precision_comparison[
                'design_supply_temperature_at_outdoor_high_b'](
                design_supply_temperature_at_outdoor_high_b.to(ureg.kelvin),
                required_supply_temperature_at_outdoor_high_b.to(ureg.
                kelvin)) and self.precision_comparison[
                'design_supply_temperature_at_outdoor_low_b'](
