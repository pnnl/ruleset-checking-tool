...
class PRM9012019Rule82a90(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule82a90, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule82a90.BoilerRule(),
            index_rmd=BASELINE_0, id='section21rule18', description=
            'For baseline building, fossil fuel systems shall be modeled using natural gas as their fuel source. Exception: For fossil fuel systems where natural gas is not available for the proposed building site as determined by the rating authority, the baseline HVAC systems shall be modeled using propane as their fuel.'
    ...
    class BoilerRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule82a90.BoilerRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), required_fields={'$': [
                'energy_source_type']}, manual_check_required_msg=
                'Baseline boiler fuel source is modeled as propane. Verify if natural gas is not available for the proposed building site as determined by the rating authority.'
                )
        ...
        def manual_check_required(self, context, calc_vals=None, data=None):
            boiler_energy_source_type_b = calc_vals[
                'boiler_energy_source_type_b']
