...
class PRM9012019Rule92f56(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule92f56, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule92f56.
            HeatingFluidLoopRule(), index_rmd=BASELINE_0, id=
            'section21rule7', description=
            'When baseline building requires boilers, systems 1,5,7,11 and 12 - Model HWST = 180F and return design temp = 130F.'
    ...
    class HeatingFluidLoopRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule92f56.HeatingFluidLoopRule, self).__init__(
                ...
                ['design_supply_temperature', 'design_return_temperature']},
                precision={'design_supply_temperature': {'precision': 0.1,
                'unit': 'K'}, 'design_return_temperature': {'precision': 
                0.1, 'unit': 'K'}})

        def get_calc_vals(self, context, data=None):
            fluid_loop_b = context.BASELINE_0
            design_supply_temperature = fluid_loop_b[
                'heating_design_and_control']['design_supply_temperature']
            design_return_temperature = fluid_loop_b[
                'heating_design_and_control']['design_return_temperature']
            return {'design_supply_temperature': CalcQ('temperature',
                design_supply_temperature), 'design_return_temperature':
                CalcQ('temperature', design_return_temperature),
                'required_supply_temperature': CalcQ('temperature',
                DESIGN_SUPPLY_TEMP), 'required_return_temperature': CalcQ(
                'temperature', DESIGN_RETURN_TEMP)}
        ...
        def rule_check(self, context, calc_vals=None, data=None):
            ...
            required_supply_temperature = calc_vals[
                'required_supply_temperature']
            required_return_temperature = calc_vals[
                'required_return_temperature']
            return design_supply_temperature.to(ureg.kelvin
                ) == required_supply_temperature.to(ureg.kelvin
                ) and design_return_temperature.to(ureg.kelvin
        ...
        def is_tolerance_fail(self, context, calc_vals=None, data=None):
            ...
            required_return_temperature = calc_vals[
                'required_return_temperature']
            return std_equal(design_supply_temperature.to(ureg.kelvin),
                required_supply_temperature.to(ureg.kelvin)) and std_equal(
