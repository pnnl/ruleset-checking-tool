...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_hvac_sys_and_assoc_zones_largest_exhaust_source import get_hvac_sys_and_assoc_zones_largest_exhaust_source
from rct229.rulesets.ashrae9012019.ruleset_functions.get_list_hvac_systems_associated_with_zone import get_list_hvac_systems_associated_with_zone
...
VENTILATION_SPACE = SchemaEnums.schema_enums[
...
APPLICABLE_SYS_TYPES = [HVAC_SYS.SYS_9, HVAC_SYS.SYS_9B, HVAC_SYS.SYS_10]
APPLICABLE_CZ_ZONES = [ClimateZoneOption.CZ0A, ClimateZoneOption.CZ0B,
    ClimateZoneOption.CZ1A, ClimateZoneOption.CZ1B, ClimateZoneOption.CZ2A,
    ClimateZoneOption.CZ2B, ClimateZoneOption.CZ3A, ClimateZoneOption.CZ3B,
...
class PRM9012019Rule07w16(RuleDefinitionListIndexedBase):
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019Rule07w16.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=True), each_rule=PRM9012019Rule07w16.RMDRule
        ...
        def create_data(self, context, data):
            ...
            for hvac_id_b in find_all(
                ...
                zone_data[hvac_id_b] = {
                    ...
                    False, 'serves_kitchen_space_p': False,
                ...
                for zone_id_b in dict_of_zones_and_terminal_units_served_by_hvac_sys_b[
                    ...
                    spaces_p = find_all(
                        ...
                        , rmd_p)
                    assert_(spaces_p,
                        f'No spaces found in zone id: {zone_id_b}.')
                    space_lighting_space_type_list_p = [space.get(
                        'lighting_space_type', None) for space in spaces_p]
                    zone_data[hvac_id_b]['serves_kitchen_space_p'] = (
                        LIGHTING_SPACE.FOOD_PREPARATION_AREA in
                        space_lighting_space_type_list_p)
                    zone_data[hvac_id_b]['all_lighting_space_types_defined_p'
                        ] = all(space_lighting_space_type_list_p)
                    space_ventilation_space_type_list_p = [space.get(
                        'ventilation_space_type', None) for space in spaces_p]
                    (zone_data[hvac_id_b][
                        'serves_zones_with_systems_likely_exhausting_toxic_etc_p'
                        ]) = (LIGHTING_SPACE.
                        LABORATORY_EXCEPT_IN_OR_AS_A_CLASSROOM in
                        space_lighting_space_type_list_p or 
                        VENTILATION_SPACE.
                    ...
                    zone_data[hvac_id_b]['serves_kitchen_space_p'] = (
                        VENTILATION_SPACE.
                        FOOD_AND_BEVERAGE_SERVICE_KITCHEN_COOKING in
                        space_ventilation_space_type_list_p)
                    zone_data[hvac_id_b][
                        'all_ventilation_space_types_defined_p'] = all(
                        space_ventilation_space_type_list_p)
                    for hvac_id_p in get_list_hvac_systems_associated_with_zone(
                        ...
                        zone_data[hvac_id_b][
                            ...
                            '$.cooling_system.dehumidification_type', hvac_p
                ...
                zone_data[hvac_id_b]['sys_type_heating_only_b'] = any([(
                    hvac_id_b in baseline_hvac_system_dict[
                    applicable_sys_type]) for applicable_sys_type in
                    APPLICABLE_SYS_TYPES])
                thermostat_heating_setpoint_schedule_list = map_(
                    dict_of_zones_and_terminal_units_served_by_hvac_sys_b[
                    hvac_id_b]['zone_list'], lambda zone_id: find_one(
                    f'$.buildings[*].building_segments[*].zones[*][?(@.id = "{zone_id}")].thermostat_heating_setpoint_schedule'
                    , rmd_p))
                assert_(thermostat_heating_setpoint_schedule_list,
                    'No thermostat heating setpoint schedule found in zones.')
                thermostat_heating_setpoint_schedule_list = list(set(
                    thermostat_heating_setpoint_schedule_list))
                thermostat_heating_setpoint_list = flat_map(
                    thermostat_heating_setpoint_schedule_list, lambda
                    schedule_id: find_exactly_one_schedule(rmd_p,
                    schedule_id).get('hourly_value', [0]))
                max_thermostat_heating_setpoint_schedule_p = max(
                    thermostat_heating_setpoint_list) * ureg('degC')
                (zone_data[hvac_id_b]['serves_zones_heated_to_60_or_higher_p']
                    ) = (max_thermostat_heating_setpoint_schedule_p >
                    REQ_HEATING_SETPOINT)
            return {'zone_data': zone_data}
        ...
        class HVACRule(RuleDefinitionBase):
            ...
            def __init__(self):
                super(PRM9012019Rule07w16.RMDRule.HVACRule, self).__init__(
                    ...
                    BASELINE_0=True, PROPOSED=True), required_fields={'$':
                    ['fan_system'], 'fan_system': ['supply_fans',
                    'minimum_outdoor_airflow', 'maximum_outdoor_airflow']},
                    precision={'OA_fraction_b': {'precision': 0.01},
            ...
            def get_calc_vals(self, context, data=None):
                ...
                ER_not_req_for_heating_sys_b = (climate_zone_b in
                    APPLICABLE_CZ_ZONES)
                ...
                hvac_fan_sys_exhaust_sum_b = zone_data[
                    'hvac_fan_sys_exhaust_sum']
                maximum_hvac_exhaust_b = zone_data['maximum_hvac_exhaust']
                fan_sys_b = hvac_b['fan_system']
                supply_airflow_b = sum([supply_fan_b.get('design_airflow',
                    ZERO.FLOW) for supply_fan_b in fan_sys_b['supply_fans']])
                min_outdoor_airflow_b = fan_sys_b['minimum_outdoor_airflow']
                max_outdoor_airflow_b = fan_sys_b['maximum_outdoor_airflow']
                OA_fraction_b = (min_outdoor_airflow_b / supply_airflow_b if
                    supply_airflow_b != ZERO.FLOW else 0.0)
                ER_modeled_b = find_one(
                    '$.fan_system.air_energy_recovery.energy_recovery_operation'
                    , hvac_b) not in (None, ENERGY_RECOVERY.NONE)
                ER_modeled_p = find_one(
                    '$.fan_system.air_energy_recovery.energy_recovery_operation'
                    , hvac_p) not in (None, ENERGY_RECOVERY.NONE)
                exception_1_applies = (sys_type_heating_only_b and not
                    serves_zones_heated_to_60_or_higher_p)
                exception_2_applies = (not ER_modeled_p and
                    serves_zones_with_systems_likely_exhausting_toxic_etc_p)
                exception_6_applies = not ER_modeled_p and max(
                    hvac_fan_sys_exhaust_sum_b, maximum_zone_exhaust_b
                    ) < 0.75 * max_outdoor_airflow_b
                exception_7_applies = (not ER_modeled_p and
                    serves_zones_that_have_dehumid_heat_recovery_p)
                return {'climate_zone_b': climate_zone_b,
                    'ER_not_req_for_heating_sys_b':
                    ER_not_req_for_heating_sys_b,
                    'serves_zones_with_systems_likely_exhausting_toxic_etc_p':
                    serves_zones_with_systems_likely_exhausting_toxic_etc_p,
                    ...
                    'hvac_fan_sys_exhaust_sum_b':
                    hvac_fan_sys_exhaust_sum_b, 'maximum_zone_exhaust_b':
                    maximum_zone_exhaust_b, 'num_hvac_exhaust_fans_b':
                    num_hvac_exhaust_fans_b, 'maximum_hvac_exhaust_b':
                    maximum_hvac_exhaust_b, 'supply_airflow_b': CalcQ(
                    'air_flow_rate', supply_airflow_b),
                    'min_outdoor_airflow_b': CalcQ('air_flow_rate',
                    min_outdoor_airflow_b), 'OA_fraction_b': OA_fraction_b,
                    'ER_modeled_b': ER_modeled_b, 'ER_modeled_p':
                    ER_modeled_p, 'max_outdoor_airflow_b':
                    max_outdoor_airflow_b, 'exception_1_applies':
                    exception_1_applies, 'exception_2_applies':
                    exception_2_applies, 'exception_6_applies':
                    exception_6_applies, 'exception_7_applies':
                    exception_7_applies}

            def manual_check_required(self, context, calc_vals=None, data=None
                ):
                OA_fraction_b = calc_vals['OA_fraction_b'].magnitude
                ...
                hvac_fan_sys_exhaust_sum_b = calc_vals[
                    'hvac_fan_sys_exhaust_sum_b']
                maximum_hvac_exhaust_b = calc_vals['maximum_hvac_exhaust_b']
                return ((OA_fraction_b > OA_fraction_b_70 or self.
                    precision_comparison['OA_fraction_b'](OA_fraction_b,
                    OA_fraction_b_70)) and (supply_airflow_b >
                    SUPPLY_AIRFLOW_5000CFM or self.precision_comparison[
                    'supply_airflow_b'](supply_airflow_b,
                    SUPPLY_AIRFLOW_5000CFM)) and not ER_modeled_b) and (not
                    ER_modeled_p and serves_kitchen_space_p or 
                    num_hvac_exhaust_fans_b > 1 and max(
                    hvac_fan_sys_exhaust_sum_b, maximum_hvac_exhaust_b) < 
                    0.75 * max_outdoor_airflow_b)
            ...
            def rule_check(self, context, calc_vals=None, data=None):
                ...
                ER_not_req_for_heating_sys_b = calc_vals[
                    'ER_not_req_for_heating_sys_b']
                ...
                return (OA_fraction_b > OA_fraction_b_70 or self.
                    ...
                    SUPPLY_AIRFLOW_5000CFM or self.precision_comparison[
                    'supply_airflow_b'](supply_airflow_b,
                    SUPPLY_AIRFLOW_5000CFM)) and (ER_modeled_b or
                    exception_1_applies or exception_2_applies or 
                    ER_not_req_for_heating_sys_b and
                    sys_type_heating_only_b or exception_6_applies or
                    exception_7_applies) or (OA_fraction_b <
                    OA_fraction_b_70 or supply_airflow_b <
                    SUPPLY_AIRFLOW_5000CFM) or (
                    all_lighting_space_types_defined_p and
                    all_ventilation_space_types_defined_p or ER_modeled_p)
            ...
            def get_fail_msg(self, context, calc_vals=None, data=None):
                ...
                return (
                    'Not all lighting or ventilation space types were defined in the RMD and therefore the potential applicability of exceptions 2 and 3 could not be fully assessed. Fail unless exceptions 2 and 3 are applicable. Exception 2 is that systems exhausting toxic, flammable, or corrosive fumes or paint or dust shall not require exhaust air energy recovery to be modeled in the baseline if it is not included in the proposed design. Exception 3 is that commercial kitchen hoods (grease) classified as Type 1 by NFPA 96 shall not require exhaust air energy recovery to be modeled in the baseline if it is not included in the proposed design.'
                     if not (all_lighting_space_types_defined_p and
                    all_ventilation_space_types_defined_p or ER_modeled_p) else
                    '')
