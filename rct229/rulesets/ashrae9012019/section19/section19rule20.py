...
class PRM9012019Rule60d49(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule60d49, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=True), each_rule=PRM9012019Rule60d49.HVACRule(),
            index_rmd=BASELINE_0, id='section19rule20', description=
    ...
    def create_data(self, context, data):
        ...
        return {'zone_supply_return_exhaust_relief_terminal_fan_power_dict_p':
            get_zone_supply_return_exhaust_relief_terminal_fan_power_dict(
            rmd_p), 'dict_of_zones_and_terminal_units_served_by_hvac_sys_b':
    ...
    class HVACRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule60d49.HVACRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), required_fields={'$': ['fan_system']
                }, precision={'hvac_sys_total_supply_fan_power_b': {
                'precision': 10, 'unit': 'W'},
                'hvac_sys_total_return_fan_power_b': {'precision': 10,
                'unit': 'W'}, 'hvac_sys_total_exhaust_fan_power_b': {
                'precision': 10, 'unit': 'W'},
                'hvac_sys_total_relief_fan_power_b': {'precision': 10,
                'unit': 'W'}})
        ...
        def get_calc_vals(self, context, data=None):
            ...
            hvac_sys_total_exhaust_fan_power_b = (
                fan_sys_power_flow_info_dict_b['exhaust_fans_power'])
            hvac_sys_total_relief_fan_power_b = fan_sys_power_flow_info_dict_b[
                'relief_fans_power']
            total_modeled_fan_power_b = (hvac_sys_total_supply_fan_power_b +
            ...
            for zone_id_b in list_zones_served_b:
                proposed_total_supply_fan_power += (
                    zone_supply_return_exhaust_relief_terminal_fan_power_dict_p
                    [zone_id_b]['supply_fans_power'])
                proposed_total_return_fan_power += (
                    zone_supply_return_exhaust_relief_terminal_fan_power_dict_p
                    [zone_id_b]['return_fans_power'])
                proposed_total_exhaust_fan_power += (
                    zone_supply_return_exhaust_relief_terminal_fan_power_dict_p
                    [zone_id_b]['exhaust_fans_power'])
                proposed_total_relief_fan_power += (
                    zone_supply_return_exhaust_relief_terminal_fan_power_dict_p
                    [zone_id_b]['relief_fans_power'])
            total_modeled_fan_power_p = (proposed_total_supply_fan_power +
                proposed_total_return_fan_power +
                proposed_total_exhaust_fan_power +
                proposed_total_relief_fan_power)
            ...
            return {'hvac_sys_total_supply_fan_power_b':
        ...
        def is_tolerance_fail(self, context, calc_vals=None, data=None):
            ...
            expected_baseline_fan_power_relief = calc_vals[
                'expected_baseline_fan_power_relief']
            return std_equal(hvac_sys_total_supply_fan_power_b,
                expected_baseline_fan_power_supply) and std_equal(
                hvac_sys_total_return_fan_power_b,
                expected_baseline_fan_power_return) and std_equal(
                hvac_sys_total_exhaust_fan_power_b,
                expected_baseline_fan_power_exhaust) and std_equal(
                hvac_sys_total_relief_fan_power_b,
                expected_baseline_fan_power_relief)
