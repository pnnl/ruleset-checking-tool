...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_fan_system_object_supply_return_exhaust_relief_total_power_flow import get_fan_system_object_supply_return_exhaust_relief_total_power_flow
...
CONSTANT_VOLUME_SYS_TYPES = [HVAC_SYS.SYS_3, HVAC_SYS.SYS_4, HVAC_SYS.
    SYS_12, HVAC_SYS.SYS_13]
VARIABLE_VOLUME_SYS_TYPES = [HVAC_SYS.SYS_5, HVAC_SYS.SYS_6, HVAC_SYS.SYS_7,
    HVAC_SYS.SYS_8]
...
class PRM9012019Rule49c09(RuleDefinitionListIndexedBase):
    ...
    def is_applicable(self, context, data=None):
        ...
        return any([(baseline_system_types_dict[system_type] and
            baseline_system_type_compare(system_type, applicable_sys_type, 
            False)) for system_type in baseline_system_types_dict for
            applicable_sys_type in APPLICABLE_SYS_TYPES])

    def create_data(self, context, data):
        rmd_b = context.BASELINE_0
        baseline_system_types_dict = get_baseline_system_types(rmd_b)
        applicable_hvac_sys_ids = [hvac_id for sys_type in
            baseline_system_types_dict for target_sys_type in
            APPLICABLE_SYS_TYPES if baseline_system_type_compare(sys_type,
            target_sys_type, False) for hvac_id in
            baseline_system_types_dict[sys_type]]
        ...
        for hvac_id_b in find_all(
            ...
            for zone_id_b in zones_and_terminal_units_served_by_hvac_sys_dict_b[
                ...
                if zonal_exhaust_fan_b:
                    zonal_exhaust_fan_elec_power_b[hvac_id_b
    ...
    class HVACRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule49c09.HVACRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), required_fields={'$': ['fan_system']
                }, precision={'total_fan_power_b': {'precision': 10, 'unit':
                'W'}})
        ...
        def get_calc_vals(self, context, data=None):
            ...
            zonal_exhaust_fan_elec_power_b = data[
                'zonal_exhaust_fan_elec_power_b'][hvac_b_id]
            sys_type_b = next((key for key, values in
                baseline_system_types_dict.items() if hvac_b_id in values),
                None)
            ...
            total_fan_power_b = (fan_sys_info_b['supply_fans_power'] +
                fan_sys_info_b['return_fans_power'] + fan_sys_info_b[
                'exhaust_fans_power'] + fan_sys_info_b['relief_fans_power'] +
                zonal_exhaust_fan_elec_power_b if exactly_one_supply_fan else
                ZERO.POWER).to(ureg.hp)
            A = 0.0
            more_than_one_exhaust_fan_and_energy_rec_is_relevant_b = False
            if fan_sys_b.get('air_energy_recovery') and fan_sys_b[
                'air_energy_recovery'].get('type'
                ) != ENERGY_RECOVERY.NONE and fan_sys_info_b['exhaust_fans_qty'
                ] > 1:
                more_than_one_exhaust_fan_and_energy_rec_is_relevant_b = True
                enthalpy_reco_ratio_b = getattr_(fan_sys_b, 'fan_system',
                    'air_energy_recovery', 'enthalpy_recovery_ratio')
                ERV_OA_air_flow_b = getattr_(fan_sys_b, 'fan_system',
                    'air_energy_recovery', 'outdoor_airflow').to(ureg.cfm)
                ERV_EX_air_flow_b = getattr_(fan_sys_b, 'fan_system',
                    'air_energy_recovery', 'exhaust_airflow').to(ureg.cfm)
                A = (2.2 * enthalpy_reco_ratio_b - 0.5) * (ERV_OA_air_flow_b +
                    ERV_EX_air_flow_b) / 4131
            MERV_rating = fan_sys_b['air_filter_merv_rating'] if fan_sys_b.get(
                'air_filter_merv_rating') is not None else 0
            if 9 <= MERV_rating <= 12:
                MERV_adj = 0.5
            elif MERV_rating > 12:
                MERV_adj = 0.9
            ...
            if any([baseline_system_type_compare(sys_type_b,
                target_sys_type, False) for target_sys_type in
                CONSTANT_VOLUME_SYS_TYPES]):
                expected_BHP_b = (0.00094 * supply_flow_b + A).m * ureg('hp')
                min_BHP_b = (0.00094 * supply_flow_b).m * ureg('hp')
            elif any([baseline_system_type_compare(sys_type_b,
                target_sys_type, False) for target_sys_type in
            ...
            min_motor_efficiency_b = table_G3_9_1_lookup(min_BHP_b)[
                'full_load_motor_efficiency_for_modeling']
            expected_fan_wattage_b = (expected_BHP_b /
                expected_motor_efficiency_b)
            min_fan_wattage_b = min_BHP_b / min_motor_efficiency_b
            return {'exactly_one_supply_fan': exactly_one_supply_fan,
        ...
        def manual_check_required(self, context, calc_vals=None, data=None):
            ...
            return (not exactly_one_supply_fan or
                more_than_one_exhaust_fan_and_energy_rec_is_relevant_b or 
                not std_equal(total_fan_power_b, expected_fan_wattage_b) and
        ...
        def get_manual_check_required_msg(self, context, calc_vals=None,
            ...
            expected_fan_wattage_b = calc_vals['expected_fan_wattage_b']
            black_word = ('' if
                more_than_one_exhaust_fan_and_energy_rec_is_relevant_b else
                'energy recovery')
            undetermined_msg = ''
            if (not exactly_one_supply_fan or
                more_than_one_exhaust_fan_and_energy_rec_is_relevant_b):
                undetermined_msg = (
                    f'{hvac_id_b} has more than one supply fan and/or more than one exhaust fan associated with the HVAC system in the baseline and therefore this check could not be conducted for this HVAC system. Conduct manual check for compliance with G3.1.2.9.'
                    )
            else:
                undetermined_msg = (
                    f'Fan power for {hvac_id_b} is greater than expected per Section and Table G3.1.2.9 assuming no pressure drop adjustments (e.g., sound attenuation, air filtration, fully ducted return when required by code, airflow control devices, carbon and other gas-phase air cleaners, coil runaround loops, evaporative humidifier/coolers in series with another cooling coil, exhaust systems serving fume hoods, and laboratory and vivarium exhaust systems in high-rise buildings) per Table 6.5.3.1-2 other than {black_word} and MERV filters defined in the RMD (if modeled). Expected Wattage = {expected_fan_wattage_b.to(ureg.kW)} kW. however not all pressure drop adjustments are able to be captured in the RMD so conduct manual check.'
                    )
        ...
        def get_fail_msg(self, context, calc_vals=None, data=None):
            ...
            return (
                f'The total fan power for <insert hvac.id> is modeled as {total_fan_power_b.to(ureg.kW)} kW which is less than the expected including pressure drop adjustments for exhaust air energy recovery and MERV filters as applicable which was calculated as {min_fan_wattage_b.to(ureg.kW)} kW .'
                )
