...
class PRM9012019Rule74p61(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule74p61, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule74p61.
            RuleSetModelInstanceRule(), index_rmd=BASELINE_0, id=
            'section19rule4', description=
            'For baseline cooling sizing runs in residential dwelling units, the infiltration, occupants, lighting, gas and electricity using equipment hourly schedule shall be the same as the most used hourly weekday schedule from the annual simulation.'
            , ruleset_section_title='HVAC - General', standard_section=
            'Section G3.1.2.2.1 Exception', is_primary_rule=True, list_path
            ='ruleset_model_descriptions[0]', required_fields={'$': [
            'calendar'], 'calendar': ['day_of_week_for_january_1']},
            data_items={'day_of_week_for_january_1': (BASELINE_0,
    ...
    class RuleSetModelInstanceRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019Rule74p61.RuleSetModelInstanceRule, self).__init__(
                ...
                PRM9012019Rule74p61.RuleSetModelInstanceRule.
                BuildingSegmentRule(), index_rmd=BASELINE_0, list_path=
        ...
        class BuildingSegmentRule(RuleDefinitionListIndexedBase):
            ...
            def __init__(self):
                super(PRM9012019Rule74p61.RuleSetModelInstanceRule.
                    BuildingSegmentRule, self).__init__(rmds_used=
                    produce_ruleset_model_description(USER=False,
                    BASELINE_0=True, PROPOSED=False), each_rule=
                    PRM9012019Rule74p61.RuleSetModelInstanceRule.
                    BuildingSegmentRule.ZoneRule(), index_rmd=BASELINE_0,
                    list_path='$.zones[*]')
            ...
            def create_data(self, context, data):
                ...
                if lighting_bldg_type_b in [LIGHTING_BUILDING_AREA.
                ...
                return {'is_lighting_bldg_area_defined_b':
            ...
            class ZoneRule(RuleDefinitionListIndexedBase):
                ...
                def __init__(self):
                    super(PRM9012019Rule74p61.RuleSetModelInstanceRule.
                        BuildingSegmentRule.ZoneRule, self).__init__(rmds_used
                        =produce_ruleset_model_description(USER=False,
                        BASELINE_0=True, PROPOSED=False), each_rule=
                        PRM9012019Rule74p61.RuleSetModelInstanceRule.
                        BuildingSegmentRule.ZoneRule.SpaceRule(), index_rmd
                        =BASELINE_0, list_path='$.spaces[*]')

                def create_data(self, context, data):
                    day_of_week_for_january_1 = data[
                    ...
                    if zone_b.get('infiltration'):
                        multiplier_sch_inf_b = getattr_(zone_b, 'Zone',
                            'infiltration', 'multiplier_schedule')
                        multiplier_sch_hourly_value_b = getattr_(
                            find_exactly_one_with_field_value('$[*]', 'id',
                            multiplier_sch_inf_b, schedule_b), 'schedule',
                            'hourly_values')
                ...
                class SpaceRule(RuleDefinitionBase):
                    ...
                    def is_applicable(self, context, data=None):
                        ...
                        return (lighting_space_type_b == LIGHTING_SPACE.
                            DWELLING_UNIT or ventilation_space_type_b ==
                            VENTILATION_SPACE.
                    ...
                    def get_calc_vals(self, context, data=None):
                        ...
                        if space_b.get('occupant_multiplier_schedule'):
                            multiplier_sch_occ_hourly_value_b = getattr_(
                                ...
                                'id', space_b[
                                'occupant_multiplier_schedule'], schedule_b
                        ...
                        for interior_lighting_b in find_all(
                            ...
                            if not int_lgt_pass_cooling_b:
                                ...
                                multiplier_sch_hourly_value_b = getattr_(
                                    find_exactly_one_with_field_value(
                                    '$[*]', 'id', multiplier_sch_light_b,
                                    schedule_b), 'schedule', 'hourly_values')
                                design_cooling_multiplier_sch_b = getattr_(
                                    find_exactly_one_with_field_value(
                                    '$[*]', 'id', multiplier_sch_light_b,
                                    schedule_b), 'schedule',
                        ...
                        for misc_equip_b in find_all(
                            ...
                            if not misc_pass_cooling_b:
                                multiplier_sch_light_b = getattr_(misc_equip_b,
                        ...
                        return {'inf_pass_cooling_b': inf_pass_cooling_b,
                            ...
                            'int_lgt_pass_cooling_b':
                            int_lgt_pass_cooling_b, 'misc_pass_cooling_b':
                            misc_pass_cooling_b}
                    ...
                    def manual_check_required(self, context, calc_vals=None,
                        ...
                        space_lighting_or_vent_space_type_not_defined = (
                            space_b.get('lighting_space_type') is None or 
                            space_b.get('ventilation_space_type') is None)
                        return (
                            space_lighting_or_vent_space_type_not_defined and
                            (not is_lighting_bldg_area_defined_b or
                            is_building_area_MF_dormitory_or_hotel_b))
                    ...
                    def rule_check(self, context, calc_vals=None, data=None):
                        ...
                        int_lgt_pass_cooling_b = calc_vals[
                            'int_lgt_pass_cooling_b']
                        misc_pass_cooling_b = calc_vals['misc_pass_cooling_b']
                        return all([inf_pass_cooling_b, occ_pass_cooling_b,
                            int_lgt_pass_cooling_b, misc_pass_cooling_b])
                    ...
                    def get_fail_msg(self, context, calc_vals=None, data=None):
                        ...
                        light_msg = ('' if int_lgt_pass_cooling_b else
                            'lighting')
                        misc_msg = ('' if misc_pass_cooling_b else
                            'gas and/or electricity miscellaneous')
                        agg_msg = ' '.join([inf_msg, occ_msg, light_msg,
                            misc_msg]).replace(' ', ', ')
                        return (
                            f'{space_id_b} does not appear to have been modeled following the rule that for baseline cooling sizing runs in residential dwelling units hourly schedules shall be the same as the most used hourly weekday schedule from the annual simulation for the following schedules: {agg_msg}.'
                            )
