...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_most_used_weekday_hourly_schedule import get_most_used_weekday_hourly_schedule
...
find_schedule = curry(lambda schedules, schedule_id, schedule_type:
    getattr_(find_exactly_one(f'$[*][?(@.id="{schedule_id}")]', schedules),
...
class PRM9012019Rule60o81(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule60o81, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule60o81.RMDRule(),
            index_rmd=BASELINE_0, id='section19rule23', description=
            ...
            'Section G3.1.2.2.1 excluding exception', is_primary_rule=True,
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019Rule60o81.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), each_rule=PRM9012019Rule60o81.
                RMDRule.BuildingSegmentRule(), index_rmd=BASELINE_0,
        ...
        class BuildingSegmentRule(RuleDefinitionListIndexedBase):
            ...
            def __init__(self):
                super(PRM9012019Rule60o81.RMDRule.BuildingSegmentRule, self
                    ).__init__(rmds_used=produce_ruleset_model_description(
                    USER=False, BASELINE_0=True, PROPOSED=False), each_rule
                    =PRM9012019Rule60o81.RMDRule.BuildingSegmentRule.
                    ZoneRule(), index_rmd=BASELINE_0, list_path='$.zones[*]')
            ...
            def create_data(self, context, data):
                ...
                is_building_area_MF_dormitory_or_hotel_b = (
                    lighting_bldg_type_b in [LIGHTING_BUILDING_AREA.
                    DORMITORY, LIGHTING_BUILDING_AREA.HOTEL_MOTEL,
                    LIGHTING_BUILDING_AREA.MULTIFAMILY])
                return {**data, 'is_lighting_bldg_area_defined_b':
            ...
            class ZoneRule(RuleDefinitionListIndexedBase):
                ...
                def __init__(self):
                    super(PRM9012019Rule60o81.RMDRule.BuildingSegmentRule.
                        ZoneRule, self).__init__(rmds_used=
                        produce_ruleset_model_description(USER=False,
                        BASELINE_0=True, PROPOSED=False), each_rule=
                        PRM9012019Rule60o81.RMDRule.BuildingSegmentRule.
                        ZoneRule.SpaceRule(), index_rmd=BASELINE_0,
                        list_path='$.spaces[*]')
                ...
                def create_data(self, context, data):
                    ...
                    if zone_b.get('infiltration'):
                        multiplier_sch_inf_id_b = getattr_(zone_b, 'Zone',
                        ...
                        max_inf_multiplier_b = max(
                            multiplier_sch_hourly_value_b)
                        inf_pass_cooling_b = every(
                            design_cooling_multiplier_sch_b, lambda
                            multiplier: multiplier == max_inf_multiplier_b)
                        inf_pass_heating_b = every(
                            design_heating_multiplier_sch_b, lambda
                            multiplier: multiplier == max_inf_multiplier_b)
                    return {**data, 'inf_pass_cooling_b':
                        inf_pass_cooling_b, 'inf_pass_heating_b':
                        inf_pass_heating_b}
                ...
                class SpaceRule(RuleDefinitionBase):
                    ...
                    def __init__(self):
                        super(PRM9012019Rule60o81.RMDRule.
                            BuildingSegmentRule.ZoneRule.SpaceRule, self
                            ).__init__(rmds_used=
                            produce_ruleset_model_description(USER=False,
                    ...
                    def get_calc_vals(self, context, data=None):
                        ...
                        find_schedule_from_schedules = data[
                            'find_schedule_from_schedules']
                        lighting_space_type_b = space_b.get(
                            'lighting_space_type')
                        ventilation_space_type_b = space_b.get(
                            'ventilation_space_type')
                        is_space_type_defined_b = (lighting_space_type_b or
                            ventilation_space_type_b)
                        is_dwelling_unit_b = (lighting_space_type_b ==
                            LIGHTING_SPACE.DWELLING_UNIT or 
                            ventilation_space_type_b == VENTILATION_SPACE.
                        ...
                        if space_b.get('occupant_multiplier_schedule'):
                            multiplier_sch_occ_b = (
                                find_schedule_from_schedules(space_b[
                                'occupant_multiplier_schedule']))
                            multiplier_sch_occ_hourly_value_b = (
                                multiplier_sch_occ_b('hourly_values'))
                            ...
                            multiplier_sch_design_heating_occ_b = (
                                multiplier_sch_occ_b(
                                'hourly_heating_design_day'))
                            max_multiplier_occ = max(
                                multiplier_sch_occ_hourly_value_b)
                            min_multiplier_occ = min(
                                multiplier_sch_occ_hourly_value_b)
                            occ_pass_cooling_b = every(
                                multiplier_sch_design_cooling_occ_b, lambda
                                multiplier: multiplier == max_multiplier_occ)
                            occ_pass_heating_b = every(
                                multiplier_sch_design_heating_occ_b, lambda
                                multiplier: multiplier == min_multiplier_occ)
                        int_lgt_pass_cooling_b = True
                        int_lgt_pass_heating_b = True
                        for interior_lighting_b in find_all(
                            '$.interior_lighting[*]', space_b):
                            if (int_lgt_pass_cooling_b or
                                ...
                                int_lgt_pass_cooling_b = (
                                    int_lgt_pass_cooling_b and every(
                                    multiplier_sch_design_cooling_light_b, 
                                    lambda x: x == max_multiplier_light))
                                int_lgt_pass_heating_b = (
                                    int_lgt_pass_heating_b and every(
                                    multiplier_sch_design_heating_light_b, 
                        ...
                        for misc_equip_b in find_all(
                            ...
                            if misc_pass_cooling_b or misc_pass_heating_b:
                                multiplier_sch_misc_b = getattr_(misc_equip_b,
                                ...
                                min_multiplier_misc = min(
                                    multiplier_sch_misc_hourly_value_b)
                                misc_pass_cooling_b = (misc_pass_cooling_b and
                                    every(
                                    multiplier_sch_design_cooling_misc_b, 
                                    lambda x: x == max_multiplier_misc))
                                misc_pass_heating_b = (misc_pass_heating_b and
                                    every(
                                    multiplier_sch_design_heating_misc_b, 
                                    lambda x: x == min_multiplier_misc))
                        return {'is_space_type_defined_b':
                            is_space_type_defined_b, 'is_dwelling_unit_b':
                            is_dwelling_unit_b,
                            'is_lighting_bldg_area_defined_b': data[
                            'is_lighting_bldg_area_defined_b'],
                            'is_building_area_MF_dormitory_or_hotel_b':
                            data['is_building_area_MF_dormitory_or_hotel_b'
                            ], 'inf_pass_cooling_b': data[
                            'inf_pass_cooling_b'], 'inf_pass_heating_b':
                            data['inf_pass_cooling_b'],
                            'occ_pass_cooling_b': occ_pass_cooling_b,
                            'occ_pass_heating_b': occ_pass_heating_b,
                            'int_lgt_pass_cooling_b':
                            int_lgt_pass_cooling_b,
                            'int_lgt_pass_heating_b':
                            int_lgt_pass_heating_b, 'misc_pass_cooling_b':
                            misc_pass_cooling_b, 'misc_pass_heating_b':
                            misc_pass_heating_b}

                    def manual_check_required(self, context, calc_vals=None,
                        data=None):
                        is_dwelling_unit_b = calc_vals['is_dwelling_unit_b']
                        is_space_type_defined_b = calc_vals[
                            'is_space_type_defined_b']
                        is_building_area_MF_dormitory_or_hotel_b = calc_vals[
                            'is_building_area_MF_dormitory_or_hotel_b']
                        is_heating_schedule_pass = all([calc_vals[
                            'inf_pass_heating_b'], calc_vals[
                            'occ_pass_heating_b'], calc_vals[
                            'int_lgt_pass_heating_b'], calc_vals[
                            'misc_pass_heating_b']])
                        return not (is_dwelling_unit_b or
                            is_space_type_defined_b or
                            is_building_area_MF_dormitory_or_hotel_b
                            ) and is_heating_schedule_pass

                    def get_manual_check_required_msg(self, context,
                        calc_vals=None, data=None):
                        is_cooling_schedule_pass = all([calc_vals[
                            'inf_pass_cooling_b'], calc_vals[
                            'occ_pass_cooling_b'], calc_vals[
                            'int_lgt_pass_cooling_b'], calc_vals[
                            'misc_pass_cooling_b']])
                        return (
                    ...
                    def rule_check(self, context, calc_vals=None, data=None):
                        ...
                        is_cooling_schedule_pass = all([calc_vals[
                            ...
                            'int_lgt_pass_cooling_b'], calc_vals[
                            'misc_pass_cooling_b']])
                        return (is_dwelling_unit_b and
                            is_heating_schedule_pass or not
                            is_building_area_MF_dormitory_or_hotel_b and
                            is_heating_schedule_pass and
                            is_cooling_schedule_pass)
                    ...
                    def get_fail_msg(self, context, calc_vals=None, data=None):
                        ...
                        is_cooling_schedule_pass = all([calc_vals[
                            ...
                            'int_lgt_pass_cooling_b'], calc_vals[
                            'misc_pass_cooling_b']])
                        confirmed_non_dwelling_heat_cool_failed = (
                            f'{space_id_b} does not appear to have followed this rule per Section G3.1.2.2.1 for one more more of the following heating or cooling design schedules: infiltration, occupants, lighting, gas and electricity using equipment'
                            )
