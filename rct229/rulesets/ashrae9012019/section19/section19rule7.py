        ...
        for space_b in find_all(
            ...
            if hvac_system_serves_only_labs:
                ...
                if lighting_space_type_b is None:
                    ...
                    if (lighting_space_type_b != LIGHTING_SPACE.
        ...
        for hvac_id_b in find_all(
            ...
            for zone_id_b in dict_of_zones_and_terminal_units_served_by_hvac_sys_b[
                ...
                zone_data[hvac_id_b]['was_DCV_modeled_baseline'] = any([
                    terminal_b.get('has_demand_control_ventilation', False) for
                    terminal_b in find_all('$.terminals[*]',
                ...
                zone_data[hvac_id_b]['was_DCV_modeled_proposed'] = any([(
                    terminal_p.get('has_demand_control_ventilation', False) or
                    find_one(
                    '$.fan_system.demand_control_ventilation_control',
                    find_exactly_one_hvac_system(rmd_p, getattr_(terminal_p,
                    'terminals',
                    'served_by_heating_ventilating_air_conditioning_system'
                    ))) not in [None, DEMAND_CONTROL_VENTILATION_CONTROL.
                    NONE]) for terminal_p in find_all('$.terminals[*]',
                    zone_p)])
                zone_data[hvac_id_b][
                    'zone_air_distribution_effectiveness_greater_than_1'
                    ] = getattr_(zone_p, 'zones',
                    'air_distribution_effectiveness') > 1
            zone_data[hvac_id_b]['aggregated_min_OA_schedule_across_zones_b'
                ] = aggregate_min_OA_schedule_across_zones(zone_data[
                hvac_id_b]['zone_OA_CFM_list_of_schedules_b'])
            zone_data[hvac_id_b]['aggregated_min_OA_schedule_across_zones_p'
                ] = aggregate_min_OA_schedule_across_zones(zone_data[
                hvac_id_b]['zone_OA_CFM_list_of_schedules_p'])
        return {'hvac_system_serves_only_labs':
            hvac_system_serves_only_labs,
            'are_any_lighting_space_types_defined':
            are_any_lighting_space_types_defined,
            'all_lighting_space_types_defined':
            all_lighting_space_types_defined, 'zone_data': zone_data}


    class HVACRule(RuleDefinitionBase):

        def __init__(self):
            super(PRM9012019Rule29n92.HVACRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=False), precision={
                'aggregated_min_OA_schedule_across_zones_b': {'precision': 
                1, 'unit': 'cfm'}})
        ...
        def is_applicable(self, context, data=None):
            ...
            return (not hvac_system_serves_only_labs or 
                hvac_system_serves_only_labs and not
                all_lighting_space_types_defined)
        ...
        def get_calc_vals(self, context, data=None):
            ...
            OA_CFM_schedule_match = all([self.precision_comparison[
                ...
                aggregated_min_OA_schedule_across_zones_p[i]) for i in
                range(len(aggregated_min_OA_schedule_across_zones_b))])
            ...
            return {'hvac_id_b': hvac_id_b, 'hvac_system_serves_only_labs':
                hvac_system_serves_only_labs,
                'are_any_lighting_space_types_defined':
                are_any_lighting_space_types_defined,
                'all_lighting_space_types_defined':
                all_lighting_space_types_defined, 'OA_CFM_schedule_match':
        ...
        def manual_check_required(self, context, calc_vals=None, data=None):
            ...
            modeled_proposed_total_zone_min_OA_CFM = calc_vals[
                'modeled_proposed_total_zone_min_OA_CFM']
            return (modeled_baseline_total_zone_min_OA_CFM >
                modeled_proposed_total_zone_min_OA_CFM and
                zone_air_distribution_effectiveness_greater_than_1 or 
                modeled_baseline_total_zone_min_OA_CFM <
                modeled_proposed_total_zone_min_OA_CFM)
        ...
        def get_manual_check_required_msg(self, context, calc_vals=None,
            ...
            hvac_system_serves_only_labs = calc_vals[
                'hvac_system_serves_only_labs']
            modeled_baseline_total_zone_min_OA_CFM = calc_vals[
                'modeled_baseline_total_zone_min_OA_CFM']
            modeled_proposed_total_zone_min_OA_CFM = calc_vals[
                'modeled_proposed_total_zone_min_OA_CFM']
            undetermined_msg = ''
            if (modeled_baseline_total_zone_min_OA_CFM >
                modeled_proposed_total_zone_min_OA_CFM):
                if not hvac_system_serves_only_labs:
                    undetermined_msg = (
                        f'For {hvac_id_b} the modeled baseline minimum ventilation system outdoor air intake flow CFM is higher than the minimum ventilation system outdoor air intake flow CFM modeled in the proposed design. It appears as though G3.1.2.5 Exception 2 may be applicable. A manual check for this exception is recommended otherwise fail.'
                        )
                else:
                    undetermined_msg = (
                        f'For {hvac_id_b} the modeled baseline minimum ventilation system outdoor air intake flow CFM is higher than the minimum ventilation system outdoor air intake flow CFM modeled in the proposed design. It appears as though G3.1.2.5 Exception 2 may be applicable because the air distribution effectiveness was modeled as greater than 1. Alternatively, the system may only serves lab spaces and G3.1.2.5 Exception 4 may be applicable. A manual check for these exceptions is recommended otherwise fail.'
                        )
            else:
                undetermined_msg = (
                    f'For {hvac_id_b} the modeled minimum ventilation system outdoor air intake flow CFM is lower than the minimum ventilation system outdoor air intake flow CFM modeled in the proposed design. Check if G3.1.2.5 Exception 3 is applicable. This exception states that where the minimum outdoor air intake flow in the proposed design is provided in excess of the amount required by the building code or the rating authority, the baseline building design shall be modeled to reflect the greater of that required by either the rating authority or the building code and will be less than the proposed design.'
                    )
        ...
        def get_pass_msg(self, context, calc_vals=None, data=None):
            ...
            if not are_any_lighting_space_types_defined:
                pass_msg = (
                    f'{hvac_id_b} passes this check unless it only serves lab spaces (no space types were defined in the RMD so this could not be determined). Outcome is UNDETERMINED if the HVAC system only serves lab spaces due to G3.1.2.5 Exception 4.'
                    )
            elif hvac_system_serves_only_labs and are_any_lighting_space_types_defined:
                pass_msg = (
                    f'{hvac_id_b} passes this check unless it only serves labs. This hvac system serves some labs but it could not be determined from the RMD if it only serves labs. Outcome is UNDETERMINED if the HVAC system only serves lab spaces due to G3.1.2.5 Exception 4.'
                    )
            return pass_msg
        ...
        def get_fail_msg(self, context, calc_vals=None, data=None):
            ...
            if (modeled_baseline_total_zone_min_OA_CFM >
                modeled_proposed_total_zone_min_OA_CFM and not
                was_DCV_modeled_baseline and was_DCV_modeled_proposed and 
                not zone_air_distribution_effectiveness_greater_than_1):
                if hvac_system_serves_only_labs:
                    Fail_msg = (
                        f'For {hvac_id_b} the baseline modeled minimum ventilation system outdoor air intake flow CFM is higher than the minimum ventilation system outdoor air intake flow CFM modeled in the proposed design. Demand-controlled ventilation was modeled in the proposed and not the baseline model and demand-controlled ventilation may be double accounted for in the model (per the HVAC controls and via reduced OA CFM rates in the proposed). Alternatively, the hvac system may only serve labs in which case G3.1.2.5 Exception 4 may be applicable and leading to allowed higher modeled rates in the baseline.'
                        )
                else:
                    Fail_msg = (
                ...
                Fail_msg = (
                    f'For {hvac_id_b} the modeled baseline minimum ventilation system outdoor air intake flow CFM is higher than the minimum ventilation system outdoor air intake flow CFM modeled in the proposed design which does not meet the requirements of Section G3.1.2.5.'
                    )
            elif std_equal(modeled_baseline_total_zone_min_OA_CFM,
                modeled_proposed_total_zone_min_OA_CFM):
                Fail_msg = (
                    f'Fail because the outdoor air schedules do not appear to match between the baseline and proposed.'
                    )
            else:
                Fail_msg = (
                    f'For {hvac_id_b} the modeled baseline minimum ventilation system outdoor air intake flow CFM is higher than the minimum ventilation system outdoor air intake flow CFM modeled in the proposed design which does not meet the requirements of Section G3.1.2.5. Fail unless the hvac system only serves labs and G3.1.2.5 Exception 4 is applicable.'
                    )
            return Fail_msg
