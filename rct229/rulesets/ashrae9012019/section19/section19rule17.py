...
APPLICABLE_SYS_TYPES = [HVAC_SYS.SYS_1, HVAC_SYS.SYS_2]
...
class PRM9012019Rule84b07(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule84b07, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule84b07.HVACRule(),
            index_rmd=BASELINE_0, id='section19rule17', description=
            'For baseline system 1 and 2, the total fan electrical power (Pfan) for supply, return, exhaust, and relief shall be = CFMs × 0.3, where, CFMs = the baseline system maximum design supply fan airflow rate, cfm.'
    ...
    def create_data(self, context, data):
        ...
        return {'baseline_system_types_dict': baseline_system_types_dict,
            ...
            'zone_exhaust_fan_power_dict_b': {zone['id']: sum([
            get_fan_object_electric_power(exhaust_fan) for exhaust_fan in
            find_all('$.zonal_exhaust_fan', zone)]) for zone in find_all(
    ...
    class HVACRule(RuleDefinitionBase):
        ...
        def get_calc_vals(self, context, data=None):
            ...
            dict_of_zones_and_terminal_units_served_by_hvac_sys = data[
                'dict_of_zones_and_terminal_units_served_by_hvac_sys']
            zone_exhaust_fan_power_dict_b = data[
            ...
            for fan_type_b in ('supply', 'return', 'relief', 'exhaust'):
                for fan_b in find_all(f'$.{fan_type_b}_fans[*]', fan_sys_b):
                    if fan_type_b == 'supply':
                        supply_airflow_b += getattr_(fan_b, 'Supply fans',
                            'design_airflow')
            ...
            fan_power_airflow = (total_fan_power / supply_airflow_b if 
                supply_airflow_b != ZERO.FLOW else ZERO.POWER_PER_FLOW)
            return {'fan_power_airflow': fan_power_airflow}

        def rule_check(self, context, calc_vals=None, data=None):
            fan_power_airflow = calc_vals['fan_power_airflow']
            return (fan_power_airflow < FAN_POWER_AIRFLOW_LIMIT or self.
                precision_comparison['fan_power_airflow'](fan_power_airflow,
