...
class PRM9012019Rule67l25(RuleDefinitionBase):
    ...
    def __init__(self):
        super(PRM9012019Rule67l25, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), id='section22rule23', description=
            'Each baseline chiller shall be modeled with separate chilled water pump interlocked to operate with the associated chiller.'
    ...
    def get_calc_vals(self, context, data=None):
        ...
        primary_chw_loop_id_array = find_all('$.chillers[*].cooling_loop',
            rmd_b)
        interlock_flag = all(find_all(
            '$.chillers[*].is_chilled_water_pump_interlocked', rmd_b))
        primary_chw_loop_pump_num = len([pump_b for pump_b in find_all(
            '$.pumps[*]', rmd_b) if pump_b['loop_or_piping'] in
            primary_chw_loop_id_array])
        return {'num_of_chillers_b': num_of_chillers_b,
    ...
    def rule_check(self, context, calc_vals=None, data=None):
        ...
        return (num_of_chillers_b == primary_chw_loop_pump_num and
            interlock_flag)
