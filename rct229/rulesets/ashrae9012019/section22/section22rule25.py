...
class PRM9012019Rule03q09(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule03q09, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule03q09.
            PrimaryCoolingFluidLoopRule(), index_rmd=BASELINE_0, id=
            'section22rule25', description=
            'Baseline chilled water loops that do not use purchased chilled water and do not serve computer rooms (i.e., do not serve baseline system type 11) shall have a constant-flow primary pump power of 9 W/gpm at design conditions.'
            , ruleset_section_title='HVAC - Chiller', standard_section=
            'Section G3.1.3.10 Chilled-water pumps (Systems 7, 8, 11, 12, and 13)'
    ...
    def is_applicable(self, context, data=None):
        ...
        available_sys_types = [hvac_type for hvac_type in
        ...
        return any([(available_type in APPLICABLE_SYS_TYPES) for
            available_type in available_sys_types]) and not any([(
            available_type in NOT_APPLICABLE_SYS_TYPES) for available_type in
            available_sys_types]) and primary_secondary_loop_dict
    ...
    class PrimaryCoolingFluidLoopRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule03q09.PrimaryCoolingFluidLoopRule, self
                ).__init__(rmds_used=produce_ruleset_model_description(USER
                =False, BASELINE_0=True, PROPOSED=False), required_fields={
                '$': ['pump_power_per_flow_rate']}, precision={
                'primary_pump_power_per_flow_rate': {'precision': 1, 'unit':
                'W/gpm'}})
        ...
        def get_calc_vals(self, context, data=None):
            ...
            return {'primary_pump_power_per_flow_rate': CalcQ(
        ...
        def is_tolerance_fail(self, context, calc_vals=None, data=None):
            ...
            required_pump_power_per_flow_rate = calc_vals[
                'required_pump_power_per_flow_rate']
            return std_equal(required_pump_power_per_flow_rate,
                primary_pump_power_per_flow_rate)
