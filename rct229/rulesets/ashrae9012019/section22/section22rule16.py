...
class PRM9012019Rule81f32(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule81f32, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule81f32.
            HeatRejectionRule(), index_rmd=BASELINE_0, id='section22rule16',
            description=
            'The baseline condenser water design supply temperature shall be calculated using the cooling tower approach to the 0.4% evaporation design wet-bulb temperature, valid for evaporation design wet-bulb temperatures from 55°F to 90°F.'
            , ruleset_section_title='HVAC - Chiller', standard_section=
            'Section G3.1.3.11 Heat Rejection (System 7, 8, 11, 12 and 13)',
    ...
    def is_applicable(self, context, data=None):
        ...
        return any([(available_type in APPLICABLE_SYS_TYPES) for
            ...
            REQUIRED_HIGH_DESIGN_WETBULB_TEMP.to(ureg.kelvin) for
            heat_rejection_wetbulb_temp_b in find_all(
            'heat_rejections[*].design_wetbulb_temperature', rmd_b))

    def create_data(self, context, data):
        rmd_b = context.BASELINE_0
        heat_rejection_loop_dict = {heat_rejection_loop:
            find_exactly_one_with_field_value('$.fluid_loops[*]', 'id',
            heat_rejection_loop, rmd_b) for heat_rejection_loop in find_all
            ('$.heat_rejections[*].loop', rmd_b)}
    ...
    class HeatRejectionRule(RuleDefinitionBase):
        ...
        def get_calc_vals(self, context, data=None):
            ...
            design_supply_temperature_b = getattr_(data[
                'heat_rejection_loop_dict'][loop_b],
                'design_supply_temperature',
                'cooling_or_condensing_design_and_control',
                'design_supply_temperature')
            return {'loop_b': loop_b, 'design_wetbulb_temperature_b': CalcQ
                ('temperature', design_wetbulb_temperature_b), 'approach_b':
                CalcQ('temperature', approach_b),
                'design_supply_temperature_b': CalcQ('temperature',
                design_supply_temperature_b)}

        def rule_check(self, context, calc_vals=None, data=None):
            design_wetbulb_temperature = calc_vals[
                'design_wetbulb_temperature_b']
            approach_b = calc_vals['approach_b']
            design_supply_temperature_b = calc_vals[
                'design_supply_temperature_b']
            return self.precision_comparison['design_supply_temperature_b'](
                design_supply_temperature_b.to(ureg.kelvin), 
                design_wetbulb_temperature.to(ureg.kelvin) + approach_b.to(
                ureg.kelvin))
