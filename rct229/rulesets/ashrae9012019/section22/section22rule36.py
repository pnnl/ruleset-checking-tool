...
class PRM9012019Rule01b91(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule01b91, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule01b91.
            PrimaryFluidlLoop(), index_rmd=BASELINE_0, id='section22rule36',
            description=
            'Baseline chilled water loops that do not use purchased chilled water shall be modeled with a constant flow primary loop and a variable flow secondary loop.'
    ...
    class PrimaryFluidlLoop(RuleDefinitionBase):
        ...
        def get_calc_vals(self, context, data=None):
            ...
            secondary_loop_ids = [secondary_loop['id'] for secondary_loop in
                secondary_loops]
            secondary_loops_flow_control = [getattr_(secondary_loop,
                'child loops', 'cooling_or_condensing_design_and_control',
                'flow_control') for secondary_loop in secondary_loops]
            return {'primary_loop_flow_control': primary_loop_flow_control,
        ...
        def rule_check(self, context, calc_vals=None, data=None):
            ...
            secondary_loops_flow_control = calc_vals[
                'secondary_loops_flow_control']
            return (primary_loop_flow_control == FluidLoopFlowControl.
                FIXED_FLOW and all(secondary_loop_flow_control ==
                FluidLoopFlowControl.VARIABLE_FLOW for
                secondary_loop_flow_control in secondary_loops_flow_control))
