...
class PRM9012019Rule30m88(RuleDefinitionBase):
    ...
    def __init__(self):
        super(PRM9012019Rule30m88, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), id='section22rule31', description=
            'The baseline chiller plant shall be modeled with the chiller quantity specified in Table G3.1.3.7, as a function of building peak cooling load.'
            , ruleset_section_title='HVAC - Chiller', standard_section=
            'Section G3.1.3.1 Type and Number of Chillers (System 7, 8, 11, 12 and 13)'
            , is_primary_rule=True, rmd_context=
            'ruleset_model_descriptions/0', required_fields={'$': ['output'
            ]}, precision={'building_peak_load_b': {'precision': 1, 'unit':
            'ton'}})
    ...
    def get_calc_vals(self, context, data=None):
        ...
        building_peak_load_b = getattr_(output_b,
            'building_peak_cooling_load', 'output_instance',
            'building_peak_cooling_load')
        if (building_peak_load_b < REQUIRED_BUILDING_PEAK_LOAD_300 or self.
            precision_comparison['building_peak_load_b'](
            ...
            target_chiller_number = max(2, math.ceil(building_peak_load_b /
                CHILLER_SIZE_800))
        return {'chiller_number': chiller_number, 'target_chiller_number':
            target_chiller_number}
