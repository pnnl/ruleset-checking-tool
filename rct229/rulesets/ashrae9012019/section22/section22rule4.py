...
APPLICABLE_SYS_TYPES = [HVAC_SYS.SYS_7, HVAC_SYS.SYS_8, HVAC_SYS.SYS_12,
    HVAC_SYS.SYS_13, HVAC_SYS.SYS_7B, HVAC_SYS.SYS_8B, HVAC_SYS.SYS_12B]
NOT_APPLICABLE_SYS_TYPES = [HVAC_SYS.SYS_11_1, HVAC_SYS.SYS_11_2, HVAC_SYS.
    SYS_11_1A, HVAC_SYS.SYS_11_2A, HVAC_SYS.SYS_11_1B, HVAC_SYS.SYS_11_1C]
...
class PRM9012019Rule13x50(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule13x50, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule13x50.
            ChillerFluidLoopRule(), index_rmd=BASELINE_0, id=
            'section22rule4', description=
            'Baseline chilled water loops that do not use purchased cooling and do not serve any computer rooms (i.e., do not serve baseline system type 11) shall have the chilled water supply temperature reset using the following schedule: 44F at outdoor dry-bulb temperature of 80F and above, 54F at 60F and below, and ramped linearly between 44F and 54F at temperature between 80F and 60F.'
            , ruleset_section_title='HVAC - Chiller', standard_section=
            'Section G3.1.3.9 Chilled-water supply temperature reset (System 7, 8, 11, 12 and 13)'
    ...
    def is_applicable(self, context, data=None):
        ...
        return any([(available_type in APPLICABLE_SYS_TYPES) for
            available_type in available_type_list]) and any([(
            available_type not in NOT_APPLICABLE_SYS_TYPES) for
    ...
    class ChillerFluidLoopRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule13x50.ChillerFluidLoopRule, self).__init__(
                ...
                'outdoor_low_for_loop_supply_reset_temperature': {
                'precision': 0.1, 'unit': 'K'},
                'loop_supply_temperature_at_outdoor_high': {'precision': 
                0.1, 'unit': 'K'}, 'loop_supply_temperature_at_outdoor_low':
                {'precision': 0.1, 'unit': 'K'}})
        ...
        def is_tolerance_fail(self, context, calc_vals=None, data=None):
            ...
            required_loop_supply_temperature_at_outdoor_low = calc_vals[
                'required_loop_supply_temperature_at_outdoor_low']
            return std_equal(outdoor_high_for_loop_supply_reset_temperature
                .to(ureg.kelvin),
                required_outdoor_high_for_loop_supply_reset_temperature.to(
                ureg.kelvin)) and std_equal(
                outdoor_low_for_loop_supply_reset_temperature.to(ureg.
                kelvin),
                required_outdoor_low_for_loop_supply_reset_temperature.to(
                ureg.kelvin)) and std_equal(
                loop_supply_temperature_at_outdoor_high.to(ureg.kelvin),
                required_loop_supply_temperature_at_outdoor_high.to(ureg.
                kelvin)) and std_equal(loop_supply_temperature_at_outdoor_low
                .to(ureg.kelvin),
                required_loop_supply_temperature_at_outdoor_low.to(ureg.kelvin)
                )
