...
class PRM9012019Rule95f90(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule95f90, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule95f90.
            HeatRejectionRule(), index_rmd=BASELINE_0, id='section22rule14',
            description=
            'The baseline heat rejection device shall have a design temperature rise of 10°F.'
    ...
    class HeatRejectionRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule95f90.HeatRejectionRule, self).__init__(
                ...
                'range']}, precision={'heat_rejection_range': {'precision':
                0.1, 'unit': 'K'}})

        def get_calc_vals(self, context, data=None):
            heat_rejection_b = context.BASELINE_0
            heat_rejection_range = heat_rejection_b['range']
            return {'heat_rejection_range': CalcQ('temperature_difference',
                heat_rejection_range), 'required_heat_rejection_range':
                CalcQ('temperature_difference', REQUIRED_TEMP_RANGE)}

        def rule_check(self, context, calc_vals=None, data=None):
            heat_rejection_range = calc_vals['heat_rejection_range']
            required_heat_rejection_range = calc_vals[
        ...
        def is_tolerance_fail(self, context, calc_vals=None, data=None):
            ...
            required_heat_rejection_range = calc_vals[
                'required_heat_rejection_range']
            return std_equal(heat_rejection_range.to(ureg.kelvin),
                required_heat_rejection_range.to(ureg.kelvin))
