...
class PRM9012019Rule60w01(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule60w01, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule60w01.
            CondensingFluidLoopRule(), index_rmd=BASELINE_0, id=
            'section22rule29', description=
            'Baseline chilled water loops that do not use purchased chilled water and do not serve computer rooms (i.e., do not serve baseline system type 11) shall have a condenser water pump power of 19 W/gpm at design conditions.'
            , ruleset_section_title='HVAC - Chiller', standard_section=
            'Section G3.1.3.11 Heat Rejection (Systems 7, 8, 11, 12, and 13)',
    ...
    def is_applicable(self, context, data=None):
        ...
        return any([(available_type in APPLICABLE_SYS_TYPES) for
            available_type in available_type_list]) and not any([(
            available_type in NOT_APPLICABLE_SYS_TYPES) for available_type in
            available_type_list])

    def create_data(self, context, data):
        rmd_b = context.BASELINE_0
        condenser_loop_pump_power_dict = {chiller['condensing_loop']:
            find_exactly_one_fluid_loop(rmd_b, getattr_(chiller, 'Chiller',
            'condensing_loop')).get('pump_power_per_flow_rate') for chiller in
            find_all('$.chillers[*]', rmd_b)}
        return {'condenser_loop_pump_power_dict':
    ...
    class CondensingFluidLoopRule(RuleDefinitionBase):
        ...
        def get_calc_vals(self, context, data=None):
            ...
            return {'pump_power_per_flow_rate': CalcQ(
                'power_per_liquid_flow_rate', pump_power_per_flow_rate),
                'required_pump_power': CalcQ('power_per_liquid_flow_rate',
                required_pump_power)}
