    ...
    class HeatRejectionRule(PartialRuleDefinition):
        def __init__(self):
            super(PRM9012019Rule04g06.HeatRejectionRule, self).__init__(
                ...
                required_fields={"$": ["loop", "rated_water_flowrate"]},
                precision={
                    "heat_rejection_efficiency_b": {"precision": 0.1, "unit": "gpm/hp"}
                },
            )
        ...
        def get_calc_vals(self, context, data=None):
            ...
            fan_shaft_power_b = (
                ...
                * getattr_(heat_rejection_b, "heat_rejections", "fan_motor_efficiency")
            )
        ...
        def get_manual_check_required_msg(self, context, calc_vals=None, data=None):
            ...
            additional_note_for_no_shaft_power_b = (
                ""
                if heat_rejection_b.get("fan_shaft_power")
                else f"*Note: The fan shaft power for {heat_rejection_b['id']} was not given. For this evaluation, the fan shaft power was calculated using a rule of thumb where fan_shaft_power = fan_motor_nameplate_power * {FAN_SHAFT_POWER_FACTOR} * fan_motor_efficiency."
            )
