...
class PRM9012019Rule79g01(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule79g01, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule79g01.
            HeatRejectionRule(), index_rmd=BASELINE_0, id='section22rule15',
            description=
            'The baseline heat rejection device shall have the approach calculated according to the equation 25.72 - (0.24*WB), valid for evaporation design wet-bulb temperatures from 55°F to 90°F.'
    ...
    class HeatRejectionRule(RuleDefinitionBase):
        ...
        def is_applicable(self, context, data=None):
            ...
            design_wetbulb_temp_b = heat_rejection_b[
                'design_wetbulb_temperature']
            return (heat_rejection_loop_b in heat_rejection_loop_ids_b and 
                TEMP_LOW_LIMIT_55F <= design_wetbulb_temp_b <=
                TEMP_HIGH_LIMIT_90F)
        ...
        def get_calc_vals(self, context, data=None):
            ...
            target_approach_b = 25.72 * ureg('degF') - 0.24 * heat_rejection_b[
                'design_wetbulb_temperature'].to(ureg.F)
            return {'approach_b': CalcQ('temperature_difference',
                approach_b), 'target_approach_b': CalcQ(
                'temperature_difference', target_approach_b)}

        def rule_check(self, context, calc_vals=None, data=None):
            approach_b = calc_vals['approach_b']
            target_approach_b = calc_vals['target_approach_b']
            return self.precision_comparison['approach_b'](target_approach_b
                .to(ureg.kelvin), approach_b)
