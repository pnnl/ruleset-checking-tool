...
class PRM9012019Rule55f82(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule55f82, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule55f82.ChillerRule(),
            index_rmd=BASELINE_0, id='section22rule22', description=
            'The baseline chiller efficiencies shall be modeled at the minimum efficiency levels for full load, in accordance with Tables G3.5.3.'
    ...
    class ChillerRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule55f82.ChillerRule, self).__init__(rmds_used
                ...
                ]}, precision={'full_load_efficiency_b': {'precision': 0.1}})
        ...
        def get_calc_vals(self, context, data=None):
            ...
            required_kw_ton_full_load_b = table_g3_5_3_lookup(compressor_type_b
                , rated_capacity_b)['minimum_full_load_efficiency']
            return {'full_load_efficiency_b': CalcQ('cooling_efficiency',
                full_load_efficiency_b), 'required_kw_ton_full_load_b':
                CalcQ('cooling_efficiency', required_kw_ton_full_load_b)}

        def rule_check(self, context, calc_vals=None, data=None):
            full_load_efficiency_b = calc_vals['full_load_efficiency_b'
                ].magnitude
            required_kw_ton_full_load_b = calc_vals[
                'required_kw_ton_full_load_b']
            required_cop_full_load_b = 1.0 / required_kw_ton_full_load_b.to(
                'kilowatt / kilowatt').magnitude
            return self.precision_comparison['full_load_efficiency_b'](
                full_load_efficiency_b, required_cop_full_load_b)
