...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_hvac_systems_5_6_serving_multiple_floors import get_hvac_systems_5_6_serving_multiple_floors
...
APPLICABLE_SYS_TYPES = [HVAC_SYS.SYS_1, HVAC_SYS.SYS_1B, HVAC_SYS.SYS_2,
    HVAC_SYS.SYS_3, HVAC_SYS.SYS_3B, HVAC_SYS.SYS_4, HVAC_SYS.SYS_5,
    HVAC_SYS.SYS_5B, HVAC_SYS.SYS_6, HVAC_SYS.SYS_6B]
...
class PRM9012019Rule34l50(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule34l50, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule34l50.HVACRule(),
            index_rmd=BASELINE_0, id='section10rule7', description=
            ...
            'Section Table G3.5.1-G3.5.6 Performance Rating Method Minimum Efficiency Requirements'
    ...
    def is_applicable(self, context, data=None):
        ...
        available_types_list_excl_5_6_multifloor = [system_type for 
            system_type, system_ids in baseline_system_types_dict.items() if
            any(system_id not in
            baseline_sys_5_6_serve_more_than_one_flr_list for system_id in
            system_ids)]
        return any([(available_type in APPLICABLE_SYS_TYPES) for
            available_type in available_types_list_excl_5_6_multifloor])
    ...
    def create_data(self, context, data):
        ...
        return {'baseline_system_types_dict': {system_type: [system_id for
            system_id in system_list if system_id not in
            baseline_sys_5_6_serve_more_than_one_flr_list] for system_type,
            system_list in baseline_system_types_dict.items() if 
            system_type in APPLICABLE_SYS_TYPES and system_list},
    ...
    class HVACRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule34l50.HVACRule, self).__init__(rmds_used=
        ...
        def get_calc_vals(self, context, data=None):
            ...
            if hvac_system_type_b in [HVAC_SYS.SYS_1, HVAC_SYS.SYS_1B,
                HVAC_SYS.SYS_2]:
                ...
                HVAC_SYS.SYS_3, HVAC_SYS.SYS_3B, HVAC_SYS.SYS_5, HVAC_SYS.
                SYS_5B, HVAC_SYS.SYS_6, HVAC_SYS.SYS_6B]:
                ...
                assert_(hvac_system_type_b == HVAC_SYS.SYS_4,
                ...
                expected_baseline_eff_data = table_g3_5_2_lookup(
                    ...
                    RatingCondition.SINGLE_PACKAGE, total_cool_capacity_mag_b)
                most_conservative_eff_b = expected_baseline_eff_data[
                    'most_conservative_efficiency']
            else:
                expected_baseline_eff_data = {'minimum_efficiency': None,
                    'efficiency_metric': None}
                most_conservative_eff_b = None
            expected_eff_b = expected_baseline_eff_data['minimum_efficiency']
            expected_eff_metric_b = expected_baseline_eff_data[
                'efficiency_metric']
            modeled_efficiency_values = cooling_system_b[
                'efficiency_metric_values']
            modeled_efficiency_metrics = cooling_system_b[
                'efficiency_metric_types']
            modeled_efficiency_b = next((eff for eff, metric in zip(
                modeled_efficiency_values, modeled_efficiency_metrics) if 
                metric == expected_eff_metric_b), None)
            return {'total_cool_capacity_b': CalcQ('capacity',
        ...
        def manual_check_required(self, context, calc_vals=None, data=None):
            ...
            is_zone_agg_factor_undefined_and_needed = calc_vals[
                'is_zone_agg_factor_undefined_and_needed']
            return (total_cool_capacity_b is None or
                is_zone_agg_factor_undefined_and_needed)
        ...
        def get_manual_check_required_msg(self, context, calc_vals=None,
            ...
            if modeled_efficiency_b == most_conservative_eff_b:
                undetermined_msg = (
                    'The cooling capacity of the system could not be determined. Check if the modeled baseline DX cooling efficiency was established correctly based upon equipment capacity and type while accounting for the potential aggregation of zones. The modeled efficiency matches the capacity bracket in Appendix G efficiency tables with the highest efficiency (i.e., most conservative efficiency has been modeled).'
                    )
            else:
                undetermined_msg = (
                    'The cooling capacity of the system could not be determined. Check if the modeled baseline DX cooling efficiency was established correctly based upon equipment capacity and type while accounting for the potential aggregation of zones.'
                    )
