...
from rct229.rulesets.ashrae9012019.data_fns.table_G3_5_2_fns import HeatPumpEquipmentType, RatingCondition, table_g3_5_2_lookup
from rct229.rulesets.ashrae9012019.data_fns.table_G3_5_4_fns import table_g3_5_4_lookup
from rct229.rulesets.ashrae9012019.data_fns.table_G3_5_5_fns import GasHeatingEquipmentType, table_g3_5_5_lookup
...
from rct229.rulesets.ashrae9012019.ruleset_functions.get_hvac_zone_list_w_area_dict import get_hvac_zone_list_w_area_by_rmd_dict
from rct229.utils.assertions import assert_
from rct229.utils.pint_utils import CalcQ
from rct229.utils.utility_functions import find_exactly_one_zone
APPLICABLE_SYS_TYPES = [HVAC_SYS.SYS_2, HVAC_SYS.SYS_3, HVAC_SYS.SYS_3A,
    HVAC_SYS.SYS_4, HVAC_SYS.SYS_9]
SINGLE_EFF_SYS_TYPES = [HVAC_SYS.SYS_2, HVAC_SYS.SYS_3, HVAC_SYS.SYS_3A,
    HVAC_SYS.SYS_9]
...
MOST_CONSERVATIVE_MSG = (
    'Check if the modeled baseline heating efficiency was established correctly based upon equipment capacity and type. The modeled efficiency matches the capacity bracket in Appendix G efficiency tables with the highest efficiency (i.e., most conservative efficiency has been modeled).'
    )
UNDEFINED_LOW_TEMP_MSG = (
...
MOST_CONSERVATIVE_HP_HIGH_TEMP_MSG = (
...
class PRM9012019Rule10p28(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019Rule10p28, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=False), each_rule=PRM9012019Rule10p28.HVACRule(),
            index_rmd=BASELINE_0, id='section10rule14', description=
            ...
            'Section G3.1.2.1 Equipment Efficiencies', is_primary_rule=True,
            list_path=
    ...
    def is_applicable(self, context, data=None):
        ...
        available_types_list = [hvac_type for hvac_type in
            baseline_system_types_dict if len(baseline_system_types_dict[
            hvac_type]) > 0]
        return any([(available_type in APPLICABLE_SYS_TYPES) for
            available_type in available_types_list])

    def create_data(self, context, data):
        rmd_b = context.BASELINE_0
        baseline_system_types_dict = get_baseline_system_types(rmd_b)
        baseline_system_zones_served_dict = {hvac_id: [
            find_exactly_one_zone(rmd_b, zone_id) for zone_id in hvac_data[
            'zone_list']] for hvac_id, hvac_data in
            get_hvac_zone_list_w_area_by_rmd_dict(rmd_b).items()}
        return {'baseline_system_types_dict': {system_type: [system_id for
            system_id in system_list] for system_type, system_list in
            baseline_system_types_dict.items() if system_type in
            APPLICABLE_SYS_TYPES and system_list},
    ...
    def list_filter(self, context_item, data):
        ...
        applicable_hvac_sys_ids = [hvac_id for sys_list in
            baseline_system_types_dict.values() for hvac_id in sys_list]
    ...
    class HVACRule(RuleDefinitionBase):
        ...
        def __init__(self):
            super(PRM9012019Rule10p28.HVACRule, self).__init__(rmds_used=
        ...
        def get_calc_vals(self, context, data=None):
            ...
            hvac_zone_list_w_area_dict_b = data[
                'baseline_system_zones_served_dict']
            zone_list_b = hvac_zone_list_w_area_dict_b[hvac_b['id']]
            is_zone_agg_factor_undefined_and_needed = False
            hvac_system_type_b = next((system_type for system_type,
                hvac_b_ids in baseline_system_types_dict_b.items() if 
                hvac_b['id'] in hvac_b_ids), None)
            ...
            if total_capacity_b is not None and hvac_system_type_b in [HVAC_SYS
                .SYS_3, HVAC_SYS.SYS_3A, HVAC_SYS.SYS_4]:
                hvac_zone_aggregation_factor = zone_list_b[0].get(
                    'aggregation_factor')
                if hvac_zone_aggregation_factor is not None:
                    total_capacity_b = (total_capacity_b /
                        hvac_zone_aggregation_factor)
                    total_capacity_mag_b = (total_capacity_mag_b /
                        hvac_zone_aggregation_factor)
                elif hvac_zone_aggregation_factor is None and hvac_system_type_b in [
                    HVAC_SYS.SYS_3, HVAC_SYS.SYS_3A
                    ] and total_capacity_mag_b >= FURNACE_CAPACITY_LOW_THREHSOLD:
                    is_zone_agg_factor_undefined_and_needed = True
                elif hvac_zone_aggregation_factor is None and hvac_system_type_b == HVAC_SYS.SYS_4 and total_capacity_mag_b >= HEATPUMP_CAPACITY_LOW_THRESHOLD:
                    is_zone_agg_factor_undefined_and_needed = True
            if hvac_system_type_b == HVAC_SYS.SYS_2:
                ...
                if total_capacity_b is None:
                    expected_baseline_eff_data = table_g3_5_5_lookup(
                        ...
                        WARM_AIR_UNIT_HEATER_GAS_FIRED,
                        MOST_CONSERVATIVE_FURNACE_SAMPLE)
                else:
                    expected_baseline_eff_data = table_g3_5_5_lookup(
                        GasHeatingEquipmentType.
                        WARM_AIR_UNIT_HEATER_GAS_FIRED, total_capacity_mag_b)
                ...
                expected_baseline_eff_data = [table_g3_5_2_lookup(
                    ...
                    MOST_CONSERVATIVE_HEATPUMP_SAMPLE), table_g3_5_2_lookup
                    (HeatPumpEquipmentType.HEAT_PUMP_AIR_COOLED_HEATING,
                ...
                expected_baseline_eff_data = [table_g3_5_2_lookup(
                    HeatPumpEquipmentType.HEAT_PUMP_AIR_COOLED_HEATING,
                    RatingCondition.SINGLE_PACKAGE, total_capacity_mag_b),
                    table_g3_5_2_lookup(HeatPumpEquipmentType.
                    HEAT_PUMP_AIR_COOLED_HEATING, RatingCondition.
                    SINGLE_PACKAGE, total_capacity_mag_b)]
            else:
                expected_baseline_eff_data = [table_g3_5_2_lookup(
                    HeatPumpEquipmentType.HEAT_PUMP_AIR_COOLED_HEATING,
                    RatingCondition.HIGH_TEMP, total_capacity_mag_b),
                    table_g3_5_2_lookup(HeatPumpEquipmentType.
                    HEAT_PUMP_AIR_COOLED_HEATING, RatingCondition.LOW_TEMP,
                    total_capacity_mag_b)]
            modeled_effs_b = list(zip(heating_system_b[
                'efficiency_metric_values'], heating_system_b[
            ...
            if hvac_system_type_b in SINGLE_EFF_SYS_TYPES:
                ...
                expected_effs_b = [(expected_baseline_eff[
                    'minimum_efficiency'], expected_baseline_eff[
                    'efficiency_metric']) for expected_baseline_eff in
                    expected_baseline_eff_data]
                modeled_effs_b = [modeled_eff_b for expected_eff_b in
                    expected_effs_b for modeled_eff_b in modeled_effs_b if 
                    modeled_eff_b[1] == expected_eff_b[1]]
                assert_(len(modeled_effs_b) > 0,
                    'No modeled efficiencies were found with the expected efficiency metrics'
                    )
                expected_effs_b = [expected_eff_b for expected_eff_b in
                    expected_effs_b if expected_eff_b[1] in [eff[1] for eff in
                    modeled_effs_b]]
            else:
                expected_effs_b = []
                ...
                expected_low_temp_eff_metric_b = expected_baseline_eff_data[1][
                    'efficiency_metric']
                modeled_high_temp_eff_b = next((eff for eff, metric in
                    modeled_effs_b if metric ==
                    expected_high_temp_eff_metric_b), None)
                modeled_low_temp_eff_b = next((eff for eff, metric in
                    modeled_effs_b if metric ==
                    expected_low_temp_eff_metric_b), None)
            return {'hvac_system_type_b': hvac_system_type_b,
                'total_capacity_b': CalcQ('capacity', total_capacity_b),
                'is_zone_agg_factor_undefined_and_needed':
                is_zone_agg_factor_undefined_and_needed, 'expected_effs_b':
                expected_effs_b, 'modeled_effs_b': modeled_effs_b,
        ...
        def manual_check_required(self, context, calc_vals=None, data=None):
            ...
            is_zone_agg_factor_undefined_and_needed = calc_vals[
                'is_zone_agg_factor_undefined_and_needed']
            if (len(modeled_effs_b) > 0 and len(expected_effs_b) > 0 and
                all(modeled_eff_b[0] == expected_eff_b[0] for modeled_eff_b,
                expected_eff_b in zip(modeled_effs_b, expected_effs_b)) and
                total_capacity_b is None or
                is_zone_agg_factor_undefined_and_needed):
                return True
            if (hvac_system_type_b == HVAC_SYS.SYS_4 and 
                modeled_high_temp_eff_b is not None and 
                modeled_high_temp_eff_b == expected_high_temp_eff_b and 
                modeled_low_temp_eff_b is not None and 
                modeled_low_temp_eff_b == expected_low_temp_eff_b and 
                total_capacity_b is None or
                is_zone_agg_factor_undefined_and_needed):
                return True
            if (hvac_system_type_b == HVAC_SYS.SYS_4 and 
                modeled_high_temp_eff_b == expected_high_temp_eff_b and 
                modeled_low_temp_eff_b is None and total_capacity_b is not
                None and not is_zone_agg_factor_undefined_and_needed):
                return True
            if (hvac_system_type_b == HVAC_SYS.SYS_4 and 
                modeled_high_temp_eff_b == expected_high_temp_eff_b and 
                modeled_low_temp_eff_b is None and total_capacity_b is None or
                is_zone_agg_factor_undefined_and_needed):
                return True
        ...
        def get_manual_check_required_msg(self, context, calc_vals=None,
            ...
            is_zone_agg_factor_undefined_and_needed = calc_vals[
                'is_zone_agg_factor_undefined_and_needed']
            if len(modeled_effs_b) > 0 and len(expected_effs_b) > 0 and all(
                modeled_eff_b[0] == expected_eff_b[0] for modeled_eff_b,
                expected_eff_b in zip(modeled_effs_b, expected_effs_b)):
                return MOST_CONSERVATIVE_MSG
            elif hvac_system_type_b == HVAC_SYS.SYS_4 and modeled_high_temp_eff_b == expected_high_temp_eff_b and modeled_low_temp_eff_b is None and total_capacity_b is not None and not is_zone_agg_factor_undefined_and_needed:
                return UNDEFINED_LOW_TEMP_MSG
            elif hvac_system_type_b == HVAC_SYS.SYS_4 and modeled_high_temp_eff_b is not None and modeled_high_temp_eff_b == expected_high_temp_eff_b and total_capacity_b is None:
        ...
        def rule_check(self, context, calc_vals=None, data=None):
            ...
            if hvac_system_type_b in SINGLE_EFF_SYS_TYPES:
                expected_effs_b = calc_vals['expected_effs_b']
                modeled_effs_b = calc_vals['modeled_effs_b']
                return (all(modeled_eff_b == expected_eff_b for 
                    modeled_eff_b, expected_eff_b in zip(modeled_effs_b,
                    expected_effs_b)) and total_capacity_b is not None and 
                    not is_zone_agg_factor_undefined_and_needed)
            else:
                expected_high_temp_eff_b = calc_vals['expected_high_temp_eff_b'
                    ]
                modeled_high_temp_eff_b = calc_vals['modeled_high_temp_eff_b']
                expected_low_temp_eff_b = calc_vals['expected_low_temp_eff_b']
                modeled_low_temp_eff_b = calc_vals['modeled_low_temp_eff_b']
                return (modeled_high_temp_eff_b == expected_high_temp_eff_b and
                    modeled_low_temp_eff_b == expected_low_temp_eff_b and 
                    total_capacity_b is not None and not
                    is_zone_agg_factor_undefined_and_needed)
