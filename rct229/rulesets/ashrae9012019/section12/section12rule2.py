...
EXPECTED_RECEPTACLE_CONTROL_SPACE_TYPES = [LIGHTING_SPACE_OPTIONS.
    OFFICE_ENCLOSED, LIGHTING_SPACE_OPTIONS.
    CONFERENCE_MEETING_MULTIPURPOSE_ROOM, LIGHTING_SPACE_OPTIONS.
    COPY_PRINT_ROOM, LIGHTING_SPACE_OPTIONS.
    LOUNGE_BREAKROOM_HEALTH_CARE_FACILITY, LIGHTING_SPACE_OPTIONS.
    LOUNGE_BREAKROOM_ALL_OTHERS, LIGHTING_SPACE_OPTIONS.
...
MANUAL_CHECK_REQUIRED_MSG_CASE_4 = (
    'A reduced schedule and automatic receptacle controls are present in the proposed design. The space type may have receptacle control requirements in Section 8.4.2. If that is the case, there should be no reduced schedule modeled.'
    )
MANUAL_CHECK_REQUIRED_MSG_CASE_7 = (
    'The proposed miscellaneous equipment schedule has reduced equivalent full load hours compared the baseline but it could not be determined if automatic receptacle controls are present in the proposed design to justify the credit.'
    )
FAIL_MSG_CASE_2 = (
...
class PRM9012019rule66e91(RuleDefinitionListIndexedBase):
    ...
    def __init__(self):
        super(PRM9012019rule66e91, self).__init__(rmds_used=
            produce_ruleset_model_description(USER=False, BASELINE_0=True,
            PROPOSED=True), each_rule=PRM9012019rule66e91.RMDRule(),
            index_rmd=BASELINE_0, id='section12rule2', description=
    ...
    class RMDRule(RuleDefinitionListIndexedBase):
        ...
        def __init__(self):
            super(PRM9012019rule66e91.RMDRule, self).__init__(rmds_used=
                produce_ruleset_model_description(USER=False, BASELINE_0=
                True, PROPOSED=True), each_rule=PRM9012019rule66e91.RMDRule
                .SpaceRule(), index_rmd=BASELINE_0, list_path=
                'buildings[*].building_segments[*].zones[*].spaces[*]')

        def create_data(self, context, data):
            rmd_b = context.BASELINE_0
            rmd_p = context.PROPOSED
            mis_equip_schedule_b_dict = {sch_id: find_exactly_one_schedule(
                rmd_b, sch_id)['hourly_values'] for sch_id in find_all(
                'buildings[*].building_segments[*].zones[*].spaces[*].miscellaneous_equipment[*].multiplier_schedule'
                , rmd_b)}
            mis_equip_schedule_p_dict = {sch_id: find_exactly_one_schedule(
                rmd_p, sch_id)['hourly_values'] for sch_id in find_all(
                'buildings[*].building_segments[*].zones[*].spaces[*].miscellaneous_equipment[*].multiplier_schedule'
                , rmd_p)}
        ...
        class SpaceRule(RuleDefinitionListIndexedBase):
            ...
            def __init__(self):
                super(PRM9012019rule66e91.RMDRule.SpaceRule, self).__init__(
                    ...
                    PRM9012019rule66e91.RMDRule.SpaceRule.
                    MiscellaneousEquipmentRule(), index_rmd=BASELINE_0,
            ...
            class MiscellaneousEquipmentRule(RuleDefinitionBase):
                ...
                def __init__(self):
                    super(PRM9012019rule66e91.RMDRule.SpaceRule.
                        MiscellaneousEquipmentRule, self).__init__(rmds_used
                ...
                def get_calc_vals(self, context, data=None):
                    ...
                    mis_equip_schedule_b_dict = data[
                        'mis_equip_schedule_b_dict']
                    mis_equip_schedule_p_dict = data[
                        'mis_equip_schedule_p_dict']
                    misc_equip_schedule_id_b = misc_equip_b.get(
                        'multiplier_schedule')
                    misc_equip_schedule_id_p = misc_equip_p.get(
                        'multiplier_schedule')
                    ...
                    schedules_comparison_output = compare_schedules(
                        misc_equip_schedule_b, misc_equip_schedule_p,
                        mask_schedule, is_leap_year)
                    return {'space_type_b': space_type_b,
                        'auto_receptacle_control_b':
                        auto_receptacle_control_b,
                        'auto_receptacle_control_p':
                        auto_receptacle_control_p, 'total_hours_matched':
                        schedules_comparison_output['total_hours_matched'],
                        'eflh_difference': schedules_comparison_output[
                        'eflh_difference'], 'hours_misc_equip_schedule_b':
                        len(misc_equip_schedule_b),
                        'hours_misc_equip_schedule_p': len(
                        misc_equip_schedule_p)}

                def manual_check_required(self, context, calc_vals=None,
                    data=None):
                    eflh_difference = calc_vals['eflh_difference']
                    auto_receptacle_control_b = calc_vals[
                        'auto_receptacle_control_b']
                    auto_receptacle_control_p = calc_vals[
                        'auto_receptacle_control_p']
                    space_type_b = calc_vals['space_type_b']
                    return (eflh_difference > 0 and (
                        auto_receptacle_control_p and space_type_b in
                        EXPECTED_RECEPTACLE_CONTROL_SPACE_TYPES and not
                        auto_receptacle_control_b) or space_type_b is None or
                        auto_receptacle_control_p is None)

                def get_manual_check_required_msg(self, context, calc_vals=
                    None, data=None):
                    auto_receptacle_control_p = calc_vals[
                        'auto_receptacle_control_p']
                    return (MANUAL_CHECK_REQUIRED_MSG_CASE_7 if 
                        auto_receptacle_control_p is None else
                        MANUAL_CHECK_REQUIRED_MSG_CASE_4)
                ...
                def rule_check(self, context, calc_vals=None, data=None):
                    ...
                    auto_receptacle_control_p = calc_vals[
                        'auto_receptacle_control_p']
                    return (total_hours_matched ==
                        hours_misc_equip_schedule_b ==
                        hours_misc_equip_schedule_p or eflh_difference > 0 and
                        auto_receptacle_control_p and space_type_b not in
                ...
                def get_fail_msg(self, context, calc_vals=None, data=None):
                    ...
                    auto_receptacle_control_p = calc_vals[
                        'auto_receptacle_control_p']
                    if (auto_receptacle_control_p and
                        auto_receptacle_control_b and eflh_difference != 0):
