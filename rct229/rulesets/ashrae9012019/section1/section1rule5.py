...
class PRM9012019Rule86h31(RuleDefinitionBase):
    ...
    def __init__(self):
        super(PRM9012019Rule86h31, self).__init__(rmds_used=
            ...
            BASELINE_180=True, BASELINE_270=True), id='section1rule5',
            description=
            'When on-site renewable energy generation exceeds the thresholds defined in Section 4.2.1.1, the methodology defined in this section shall be used to calculate the PCI.'
    ...
    def is_applicable(self, context, data=None):
        ...
        assert_(len(pbp_nre_set) >= 1,
            'At least one `pbp_nre_set` value must exist.')
        assert_(bbp_set[0] > 0,
    ...
    def get_calc_vals(self, context, data=None):
        ...
        assert_(len(pci_target_set) >= 1,
            'At least one `pci_target_set` value must exist.')
        return {'pbp_set': pbp_set, 'bbp_set': bbp_set, 'pbp_nre_set':
            pbp_nre_set, 'pci_set': pci_set, 'pci_target_set': pci_target_set}
    ...
    def rule_check(self, context, calc_vals=None, data=None):
        ...
        return len(pbp_set) == len(bbp_set) == len(pbp_nre_set) == len(pci_set
            ) == len(pci_target_set) == 1 and pci_set[0] + (pbp_nre_set[0] -
            pbp_set[0]) / bbp_set[0] - APPLICABLE_LIMIT <= pci_target_set[0]
