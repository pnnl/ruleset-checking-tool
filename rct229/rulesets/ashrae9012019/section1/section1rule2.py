...
class PRM9012019Rule63e94(RuleDefinitionBase):
    ...
    def __init__(self):
        super(PRM9012019Rule63e94, self).__init__(rmds_used=
            ...
            BASELINE_180=True, BASELINE_270=True), id='section1rule2',
            description=
            'The performance of the proposed design is calculated in accordance with Standard 90.1-2019 Appendix G, where Performance Cost Index = Proposed building performance (PBP) /Baseline building performance (BBP), where both the PBP and the BBP include all end-use load components associated with the building when calculating the Performance Cost Index (PCI).'
            , ruleset_section_title='Performance Calculations',
            standard_section='Section G1.2.2', is_primary_rule=True,
    ...
    def get_calc_vals(self, context, data=None):
        ...
        assert_(len(pci_set) >= 1,
            'At least one `performance_cost_index` value must exist.')
        assert_(len(pbp_set) >= 1,
            'At least one `total_proposed_building_energy_cost_including_renewable_energy` value must exist.'
            )
        assert_(len(bbp_set) >= 1,
