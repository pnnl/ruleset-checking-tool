...
class PRM9012019Rule88z11(RuleDefinitionBase):
    ...
    def __init__(self):
        super(PRM9012019Rule88z11, self).__init__(rmds_used=
            ...
            BASELINE_180=True, BASELINE_270=True), id='section1rule3',
            description=
            'The Performance Cost Index-Target (PCIt) shall be calculated using the procedures defined in Section 4.2.1.1. The PCIt shall be equal to [baseline building unregulated energy cost (BBUEC) + BPF x baseline building regulated energy cost (BBREC)]/ BBP'
            , ruleset_section_title='Performance Calculations',
            standard_section='Section 4.2.1.1', is_primary_rule=True,
    ...
    def get_calc_vals(self, context, data=None):
        ...
        for rmd in (rmd_u, rmd_b0, rmd_b90, rmd_b180, rmd_b270, rmd_p):
            if rmd is not None:
                pci_target_set.append(find_one(
                    '$.output.performance_cost_index_target', rmd))
                bpf_set.append(find_one(
                    '$.output.total_area_weighted_building_performance_factor',
                    rmd))
                bbp_set.append(find_one(
                    '$.output.baseline_building_performance_energy_cost', rmd))
                bbrec_set.append(find_one(
                    '$.output.baseline_building_regulated_energy_cost', rmd))
                bbuec_set.append(find_one(
                    '$.output.baseline_building_unregulated_energy_cost', rmd))
        pci_target_set = list(set(filter(lambda x: x is not None,
            pci_target_set)))
        ...
        assert_(len(bbrec_set) >= 1,
            'At least one `bbrec_set` value must exist.')
        assert_(len(bbuec_set) >= 1,
            'At least one `bbuec_set` value must exist.')
        assert_(bbp_set[0] > 0,
            'The `baseline_building_performance_energy_cost` value must be greater than 0.'
            )
        return {'pci_target_set': pci_target_set, 'bpf_set': bpf_set,
            'bbp_set': bbp_set, 'bbrec_set': bbrec_set, 'bbuec_set': bbuec_set}
    ...
    def rule_check(self, context, calc_vals=None, data=None):
        ...
        return len(pci_target_set) == len(bpf_set) == len(bbp_set) == len(
            bbrec_set) == len(bbuec_set) == 1 and std_equal((bbuec_set[0] +
            bpf_set[0] * bbrec_set[0]) / bbp_set[0], pci_target_set[0])
