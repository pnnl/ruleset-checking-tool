...
def _get_osstd_entry(building_area_type):
    entries = [
        entry
        for entry in _osstd_prm_interior_lighting_data
        if entry["lpd_space_type"] == building_area_type
    ]
    #returns a list of lpd space type example: ["automotive"]
    try:
        if len(entries) == 1:
            match = entries[0]
        else:
            ...
            print("Value for match doesn't exist") 
    return match
...
def table_G3_8_lpd(building_area_type):
    ...
    """
    osstd_entry = _get_osstd_entry(building_area_type)
    watts_per_sqft = osstd_entry["w/ft^2"]
    #watts_per_ft = osstd_entry["w/ft"]

    #if watts_per_sqft is None:
    lpd = watts_per_sqft
