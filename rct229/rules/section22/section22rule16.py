...
from rct229.utils.jsonpath_utils import find_all, find_exactly_one_with_field_value, find_one
...
class Section22Rule16(RuleDefinitionListIndexedBase):
    ...
    def create_data(self, context, data):
        ...
        b = find_one("$..fluid_loops[*].id", "Chiller Loop 1")
        a = find_exactly_one_with_field_value("$..fluid_loops[*].cooling_or_condensing_design_and_control", "design_supply_temperature", "Chiller Loop 1", rmi_b)
    ...
    class ChillerHeatRejectionRule(RuleDefinitionBase):
        ...
        def get_calc_vals(self, context, data=None):
            ...
            cw_loop_id_b = heat_rejection_b["loop"]
            supply_temperature_b = 1


            return {
                "wbt_b": CalcQ("temperature", wbt_b),
                "approach_b": CalcQ("temperature", approach_b),
                "supply_temperature_b": CalcQ("temperature", supply_temperature_b),
            }

        def rule_check(self, context, calc_vals=None, data=None):
            wbt_b = calc_vals["wbt_b"]
            approach_b = calc_vals["approach_b"]
            supply_temperature_b = calc_vals["supply_temperature_b"]
            return std_equal(
                supply_temperature_b.to(ureg.kelvin),
                wbt_b.to(ureg.kelvin) + approach_b.to(ureg.kelvin),
            )
