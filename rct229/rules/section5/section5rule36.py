...
class Section5Rule36(RuleDefinitionListIndexedBase):
    ...
    class BuildingRule(RuleDefinitionListIndexedBase):
        ...
        def create_data(self, context, data=None):
            ...
            return {
                **data,
                "scc_dictionary_b": get_surface_conditioning_category_dict(data["climate_zone"], baseline),
        ...
        class BuildingSegmentRule(RuleDefinitionListIndexedBase):
            ...
            def create_data(self, context, data=None):
                ...
                return {
                    ...
                    "total_skylight_area_b": data["skylight_roof_areas_dictionary_b"][baseline["id"]]["total_skylight_area"],
                    "total_skylight_area_p": data["skylight_roof_areas_dictionary_p"][proposed["id"]]["total_skylight_area"]
                }
            ...
            def create_context_list(self, context, data=None):
                ...
                return [
                    ...
                    if get_opaque_surface_type(surface_b) == OST.ROOF and scc[surface_b["id"]] != SCC.UNREGULATED
                ]

            class SurfaceRule(RuleDefinitionBase):
                def __init__(self):
                    super(Section5Rule36.BuildingRule.BuildingSegmentRule.SurfaceRule, self).__init__(
                ...
                def get_calc_vals(self, context, data=None):
                    ...
                    total_skylight_area_surface_b = sum([subsurface.get("glazed_area", ZERO.AREA) + subsurface.get("opaque_area", ZERO.AREA) for subsurface in find_all("subsurfaces[*]", roof_b)], ZERO.AREA)
                    total_skylight_area_surface_p = sum([subsurface.get("glazed_area", ZERO.AREA) + subsurface.get("opaque_area", ZERO.AREA) for subsurface in find_all("subsurfaces[*]", roof_p)], ZERO.AREA)
                ...
                def rule_check(self, context, calc_vals=None, data=None):
                    total_skylight_area_surface_b = calc_vals["total_skylight_area_surface_b"]
                    total_skylight_area_b = calc_vals["total_skylight_area_b"]
                    total_skylight_area_surface_p = calc_vals["total_skylight_area_surface_p"]
                    total_skylight_area_p = calc_vals["total_skylight_area_p"]
                    return std_equal(total_skylight_area_surface_b / total_skylight_area_b, total_skylight_area_surface_p / total_skylight_area_p)
