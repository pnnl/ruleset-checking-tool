...
from rct229.ruleset_functions.compare_schedules import (compare_schedules, LEAP_YEAR_HOURS, REGULAR_YEAR_HOURS)
...
class Section5Rule45(RuleDefinitionListIndexedBase):
    ...
    def list_filter(self, context_item, data=None):
        ...
        return (
                zone_p.get("infiltration") is not None
        )
    ...
    class ZoneRule(RuleDefinitionBase):
        ...
        def get_calc_vals(self, context, data=None):
            ...
            mask_schedule = [1.0] * (LEAP_YEAR_HOURS if data["is_leap_year"] else REGULAR_YEAR_HOURS)

            infiltration_p = zone_p["infiltration"]
            if zone_b:
                # make sure matched baseline zone is not None (required_field does not check None)
                infiltration_b = zone_b.get["infiltration"]
                compare_schedules_result_dictionary = compare_schedules(infiltration_b["multiplier_schedule"], infiltration_p["multiplier_schedule"], mask_schedule, 1.0, data["is_leap_year"])

            return {
                "total_hours_compared": compare_schedules_result_dictionary["total_hours_compared"],
                "total_hours_match": compare_schedules_result_dictionary["total_hours_match"],
                "eflh_difference": compare_schedules_result_dictionary["elfh_difference"]
            }

        def rule_check(self, context, calc_vals=None, data=None):
            total_hours_compared = calc_vals["total_hours_compared"]
            total_hours_match = calc_vals["total_hours_match"]
            # total_hours_compared needs to be greater than 0.0
            return total_hours_compared > 0.0 and total_hours_compared == total_hours_match
